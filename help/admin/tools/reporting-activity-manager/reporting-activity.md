...
1. 查看有关每个报告包的利用率信息。 表中显示的数据代表页面上次加载时报告包的报告活动。
   ...
   | **[!UICONTROL 虚拟报告包]** | 显示馈送入此基础报告包的所有虚拟报告包。 由于应用了额外层级的筛选和分段，虚拟报告包增加了报告请求的复杂性。 所有来自虚拟报告包的请求合并归入基础报告包。 |
...
队列图表显示过去 2 小时内选定报告包的报告请求平均队列等待时间（以秒为单位）。
...
| [!UICONTROL **项目**] | 保存的Workspace项目名称、API报表ID等。（元数据可能因各种应用程序而异。） |
