...
description: 如何在Adobe Analytics中删除机器人
...
此外，由于机器人发展较快，因此 Adobe 提供了其他几个强大的功能，如果定期将这些功能合理组合在一起，将有助于提高其数据质量。这些功能包括：Experience Cloud ID 服务、分段、Data Warehouse、客户属性和虚拟报表包。以下是您可以如何使用这些工具的概述。

### 步骤 1：将访客的 Experience Cloud ID 传递到新声明的 ID

要进行开始，请在[People Core Service](https://experienceleague.adobe.com/docs/core-services/interface/audiences/audience-library.html)中新建声明的ID。 将访客的Experience CloudID传递到此新声明的ID中，使用[Adobe Experience Platform Launch](https://experienceleague.adobe.com/docs/launch/using/extensions-ref/adobe-extension/id-service-extension/overview.html)可以快速轻松地完成此操作。 我们使用名称“ECID”作为声明的 ID。

![](assets/bot-cust-attr-setup.png)

以下介绍了如何通过数据元素捕获此 ID。请务必将Experience CloudOrgID正确填充到数据元素中。

```return Visitor.getInstance("REPLACE_WITH_YOUR_ECORG_ID@AdobeOrg").getExperienceCloudVisitorID();```

设置此数据元素后，请按照[这些说明](https://experienceleague.adobe.com/docs/launch/using/extensions-ref/adobe-extension/id-service-extension/overview.html)将声明的ID传递到Adobe启动中的ECID工具。
...
既然您已使用区段识别了机器人程序，下一步就是使用Data warehouse提取与此区段关联的所有Experience CloudID。 此屏幕截图显示您应如何设置[Data warehouse](/help/export/data-warehouse/data-warehouse.md)请求：
...
data warehouse报告到达后，您将拥有ECID列表，必须从历史数据中过滤。 将这些 ECID 复制并粘贴到一个空白的 .CSV 文件中，该文件只包含两列：ECID 和机器人标记。

* **ECID**:确保此列标题与您为上面新声明的ID指定的名称匹配。
* **机器人标志**:将“机器人标志”添加为“客户属性”模式维。
...
在您的数据集得到处理并集成到Analysis Workspace中后，再创建一个区段，以利用您新的“机器人标志”客户属性维度和[!UICONTROL 排除]容器:

![](assets/bot-filter-seg2.png)

### 步骤 6：使用此区段作为虚拟报表包过滤器

最后，创建一个[虚拟报表包](/help/components/vrs/vrs-about.md)，它使用此区段过滤已识别的机器人程序：

![](assets/bot-vrs.png)

此新分段的虚拟报表包现在将生成一套更简洁的数据，并删除已识别的机器人程序。
