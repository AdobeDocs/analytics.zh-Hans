...
本详细文档介绍推销eVar背后的概念，这些eVar处理和分配数据的方式与标准eVar不同。 它还说明推销eVar与产品查找方法有何关联。
...
如前所述，所有具有转化变量语法的推销eVar仅具有“最近（上次）”分配。  以下是“分配”设置对促销eVar的实际含义：如前所示，此设置不确定在访客继续使用网站时将哪些值插入`post_evar`列中。 而是，促销eVar的“分配”设置可确定哪些eVar值绑定到产品，以及这些产品如何将成功事件分配回它们绑定的eVar值。

让我们讨论一下，如果促销eVar的“分配（即绑定）”设置设置为等于“原始值（第一个值）”，会发生什么情况。 任何与`post_evar`列一起设置的产品，以及之前未绑定到post_evar列相应的“preprocessed”eVar的产品，都将绑定到`post_evar`列中包含的值。 根据报表包设置中的“过期时间”设置，在eVar值和产品之间的此绑定将永远不变，直到eVar过期为止。
...
促销eVar的过期设置允许您选择产品/eVar绑定应何时过期，以及在将eVar传递到图像请求后不应再自动填充post_evar列。 如果记录了成功事件（由您选择）或经过了特定时间段（同样是您选择的时间段），则eVar可能会过期。 Adobe Analytics每次只允许设置一个过期时间eVar。

对于产品查找方法解决方案，设置促销eVar过期时间的最佳实践应该是将其设置为等于某个产品在网站购物车中停留的时间，然后该产品才会自动从购物车中删除，或者是当发生购买事件时。 无论设置是否过期，访客购买的任何产品都会将订单/单位/收入信用分配给当时产品绑定的促销eVar值。

### 类型

eVar类型设置确定要插入到eVar中的数据类型。 在设置促销eVar时的大多数情况下（如果不是全部），此值应等于“文本”。 对促销eVar使用类型“计数器”的情况很少见，但根据跟踪需求，可以有效地利用此类型来按产品将成功分配给计数器eVar值。  使用“计数器”类型讨论解决方案不在本文档的范围之内。

### 促销捆绑事件

促销捆绑事件设置允许您指定导致产品绑定到促销eVar值的条件。 这些条件仅限于触发特定成功事件或eVar;触发流量变量（例如prop）对促销绑定没有影响。

有关促销捆绑事件设置的一个更有用的功能是，能够通过多个事件将产品与eVar值绑定。 例如，设置可允许产品通过产品查看事件、购物车添加事件或购买事件绑定到促销eVar值。 当与产品包含在同一图像请求中的任何其他事件/eVar（推销或其他事件/）时，该设置甚至可以（默认情况下）将产品绑定到推销eVar值。
...
在许多可用的设置组合中，您可能会想知道哪些设置是“最佳实践”。

如果要将“内部关键词搜索”绑定到产品ID 12345，则将按如下方式设置产品变量：

`s.products=";12345;;;;eVar1=internal keyword search";`

与productID 12345同时捕获的任何成功事件（购物车加货、购买）将计入产品ID 12345和“内部关键词搜索”的`eVar1`值。 对于与产品ID 12345关联的成功事件，其他`eVar1`值获得点数的唯一方法是：如果稍后在产品变量中将`eVar1`设置为&#x200B;**不同的**&#x200B;值(与产品ID 12345一起)。 示例：

`s.products=";12345;;;;eVar1=internal campaign";`

此配置将产品ID 12345的绑定从“内部关键词搜索”的`eVar1`值更改为“内部促销活动”的`eVar1`值。 每次使用产品语法并将eVar的分配（绑定）设置设置为“最近（上次）”时，都会重新绑定。 如果将“分配（绑定）”设置改为“原始值（第一个值）”，会发生什么情况？ 然后，将eVar1设置为等于产品ID 12345旁边的“内部促销活动”，不会将产品ID 12345重新绑定到“内部促销活动”的eVar1值。 绑定将保留为最初绑定的值 — “内部关键词搜索”。
...
* 将产品查找方法详细信息（如上所示）从查找方法页面传递到产品详细信息页面，并* *从上一页传递的项目组合相同的产品变量值。

此解决方案需要高度的复杂性，而这些复杂性可能是不可行的。
...
例如，许多服装产品具有“子SKU”，可指定尺寸、颜色、样式以及任何其他属性。 这些属性将单个子产品与属于同一父产品的其他同级产品分开。 假设您决定购买一件中蓝色的T恤和一件大红色T恤。 假定两件衬衫的父产品ID都为“tshirt123”，并且eVar10已配置为捕获子SKU。 在购买确认页面上设置的变量将设置如下：
...
使用“最近（最后）”的“分配（绑定）”设置，您可能会遇到其他问题。 在许多Web浏览体验中，访客会“重新查找”他们已查看和/或添加到购物车的产品。 这通常通过后续访问或在他们决定完成购买之前发生。 假设在首次访问网站期间，他们通过“沙漠”的关键字搜索找到了“sandal123”产品。 他们立即从关键词搜索结果页面将其添加到购物车。 捕获购物车添加的代码将按如下方式设置：
...
现在，假设访客在此次访问中未购买产品，但在三天后又返回网站。 他们知道他们已经把“sandals123”产品添加到购物车。 但是，在购买之前，他们仍希望了解更多相关信息。 访客浏览网站时，不会使用关键词搜索来查找产品。 在“重新查找”产品之前，他们最终进入“女士>鞋>凉鞋”促销浏览部分。 当它们最终“重新查找”“sandal123”产品的产品详细信息页面时，变量将按如下方式设置（在页面加载时）：
...
由于我们希望在产品交互发生时（即产品已“找到”），将“产品查找方法”eVar值绑定到产品，因此，可以安全地假定最常见的“找到的产品”交互是产品查看（访客进入产品详细信息页面时）或购物车添加（访客直接从产品查找方法页面将产品添加到购物车时）。  因此，我们可以选择这两个事件(prodView、scAdd)作为“基本”促销捆绑事件。
每当这些捆绑事件中的任何一个包含在图像请求中时，包含在同一请求中（在产品变量内）且尚未绑定到促销eVar的任何产品ID都将绑定到传递到促销eVar中的最新值（如post_evar列中所包含）。 如果将“分配（绑定）”设置设置为等于“原始值（第一个值）”，则在发生此原始绑定后重新绑定这些产品的任何尝试都将被忽略。

### 最佳实践设置

以下是最佳实践设置。 他们尽可能轻松地以最强大的结果集实现产品查找方法。 Adobe建议客户按照以下方式配置每个产品查找方法推销eVar（通常）：
...
考虑post_evar列，Adobe处理服务器将按以下方式查看此点击：
...
post_products列中包含的值可能您很熟悉。 在本文档中向上滚动，并比较此post_products值与s.products值，如下所示。  您会注意到post_products列是使用产品变量语法设置的！ 这意味着通过产品语法将转化变量语法eVar值“复制”到产品变量中。 仅当产品变量和捆绑事件(通过eVar配置设置)包含在同一请求中时，才会执行此复制操作。 此时，post_eVar列中包含的值将绑定到产品。 此绑定通过存储在post_products列中的产品语法来表示。
