...
**[!UICONTROL 设置渠道的值]** 定义可用于该渠道的营销渠道详细信息维度。
...
>[!NOTE]
>
>您定义的任何文本字段，例如查询字符串参数或要匹配的值列表，均评估为 **区分大小写** 值。 例如，如果您有一个规则，其中的查询字符串参数 `cmp = abc123`，两者的全部大写和小写变体 `cmp` 和 `abc123` 匹配。

| 术语 | 定义 |
|--- |--- |
| 全部 | 仅当规则中的所有条件均为true时才激活此渠道。 |
| 任何 | 当规则中的任何条件为true时，激活此渠道。 仅当规则中存在多个条件时，此选项才可用。 |
| AMO ID | Advertising Cloud 和 Advertising Analytics 集成使用的主要跟踪代码。如果启用其中一个集成，可以使用跟踪代码前缀来标识特定于 Advertising Cloud 的渠道。对于搜索，使用以“AL”开头的“AMO ID”；对于显示，使用以“AC”开头的“AMO ID”；对于社交，使用以“AO”开头的“AMO ID”。在营销渠道中使用AMO ID时，点击/成本/展示次数量度可归因于正确的渠道（未配置时，这些量度将转至“直接”或“无”）。 |
| AMO ED ID | Advertising Cloud 使用的辅助跟踪代码。此跟踪代码的主要用途是用作将数据发送回 Ad Cloud 的键值。但是，如果您希望将显示点进次数和显示视图点进次数作为两个单独的营销渠道，还可以使用此代码来标识它们。可以通过为以结尾的“AMO EF ID”设置营销渠道逻辑来完成此操作 `:d` （对于显示点进次数）或结尾为 `:i` 用于显示“视图到达”。 如果您不希望将显示拆分为两个渠道，可以改为使用 AMO ID 维度。 |
| 转化变量 | 由为本报表包启用的 eVar 组成，并且只在通过网页上的 Adobe 代码设置变量时才适用。 |
| 存在 | 可使用若干选项，包括：<ul><li>**不存在**：指定请求上不存在点击属性。例如在反向链接域中，如果用户键入一个 URL 或单击一个书签，则该反向链接域属性不存在。</li><li>**为空**：指定点击属性存在，通常为 eVar 或查询字符串参数，但没有任何与点击属性相关的值。</li><li>**不包含**：例如，您可以指定反向链接域不包含特定值（与使用“包含”选项相反）。</li></ul> |
...
| 引用网站与内部 URL 过滤器不匹配 | 按照“管理工具”中对报告包的定义，引用 URL 与内部 URL 筛选器不匹配。您可以将此设置与页面URL和存在一起使用来设置一个通用规则，这样就不会有访问登陆报表中的未识别渠道区域。 |
...
| 页面 URL | 网站上网页的 URL。 |
| 反向链接域 | 此 [反向链接域](/help/components/dimensions/referring-domain.md) 维度 |
| 查询字符串参数 | 使用单个查询字符串参数。 每个条件只能指定一个查询字符串参数。 要添加其他查询字符串参数，请使用 `ANY` 作为您的运算符，然后将查询字符串参数添加到规则中。 |
...
| 搜索引擎 + 关键词 | 唯一识别搜索引擎的搜索关键词和搜索引擎组合。例如，如果搜索单词computer，则搜索引擎和关键字的标识如下： `Search Tracking Code = "<search_type>:<search engine>:<search keyword>" where    search_type = "n" or "p", search_engine = "Google", and search_keyword = "computer"`**注意：** n =免费； p =付费 |
| 将渠道值设置为 | 设置 [营销渠道详细信息](/help/components/dimensions/marketing-detail.md) 维度。 您可以确定在规则上下文中哪个值最好。 示例包括横幅广告ID、搜索关键词或电子邮件促销活动。 |
