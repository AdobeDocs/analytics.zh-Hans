...
使用通配符匹配同一块中的一个IP地址或多个地址(&#42;)。 提供您要匹配的 IP 地址的数值。替代 &#42; 对于要使用通配符进行匹配的任何值。 下表包含 IP 地址匹配字符串的示例：
...
提供 IP 地址的起始和终止范围来进行匹配。替代 &#42; 对于要使用通配符进行匹配的任何值。
...
VISTA规则在机器人规则之后应用。 请参阅 [处理顺序](/help/technotes/processing-order.md) 技术说明用户指南中的。
...
如果删除了最后八位字节，则该操作是在 IP 过滤之前完成的。这样，最后八位字节将被替换为 0，并且应当更新 IP 排除规则以匹配末尾为 0 的 IP 地址。匹配 &#42; 应与0匹配。
