...
移动设备查找[维度](overview.md)提供了关于访问您网站的移动设备属性的洞察。 这些属性基于用户代理和点击的 IP 地址。 您可以使用这些维度来了解移动设备支持哪些功能。
...
* **[!UICONTROL 移动设备屏幕大小]**：移动设备的完整尺寸（以像素为单位）。 报告的屏幕大小不指示设备的方向。 无论屏幕方向如何，每台设备在报表中的屏幕分辨率都是固定的。 这种大小基于确定哪个方向更有可能发生变化的研究。 您可能会在同一报表中看到 `"768x1024"` 和 `"1024x768"` 等大小，每种大小代表一个或多个不同的设备。
