...
“推荐人类型”维度报告哪些通用渠道访客点进来到您的站点。 Adobe维护每个维度值的规则，与营销渠道 [不同](marketing-channel.md)，您的组织维护每个渠道的规则。
...
* **“搜索引擎”维值中的搜索引擎域**: 该推荐人符合Adobe分类为搜索引擎的所有条件。 引用域是有效的搜索引擎， *引用* URL包含关键字查询字符串参数。
* **“其他网站”维值中的搜索引擎域**: 引用URL未满足要分类为搜索引擎的所有条件。 常见示例包括专用于搜索之外的其他功能的子域。 例如， `mail.google.com` 或 `autos.yahoo.com` 不是搜索引擎，但驻留在通常与搜索关联的顶级域上。 这些子域不包含关键字查询字符串，这就是它们包含在“其他网站”而不是“搜索引擎”下的原因。
