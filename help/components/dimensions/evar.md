...
报告中使用的大写或小写基于您在某个给定的自然月中发送的第一个值。 取决于报告窗口以及此时段内首次收集的 eVar 值的情况，大小写可能会发生变化。
...
1. 访客通过您的主页来到您的网站。
2. 他们使用您网站的内部搜索功能搜索“cats”。 您的实施使用 eVar1 进行内部搜索。
3. 他们查看产品，并完成结账过程。
...
Adobe Analytics 中的工具（如 Analysis Workspace）可以处理此收集的数据。 例如，如果您使用 eVar1 作为维度，使用“订单”作为量度提取报告，则会看到以下类似报告：

| `Internal search term (eVar1)` | `Orders` |
| --- | --- |
| `cats` | `1` |

Analysis Workspace 使用以下逻辑提取此报告：
...
由于分配和过期决定了哪些值会保留，因此它们对于充分利用分析实施至关重要。 Adobe 强烈建议您在组织内讨论如何处理（分配）每个 eVar 的多个值以及 eVar 停止保留数据（有效期限）的时间。

* 默认情况下，eVar 使用上一次分配。 新值将覆盖保留值。
* 默认情况下，eVar 的过期时间为访问结束时。 访问结束后，值将停止在 `post_evar` 列中逐行复制。
...
* 默认情况下，Prop 不会在设置它们的那次点击之后继续保留。 eVar 具有自定义过期时间，允许您确定 eVar 何时不再归因于后续事件。 但是，如果您使用[报表时间处理](/help/components/vrs/vrs-report-time-processing.md)，则 prop 和 eVar 均可使用自定义归因模型。
