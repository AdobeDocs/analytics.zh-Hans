...
* 基于退出和条目的访客：来自Facebook的访客、定义的登陆页面、引用域。
* 基于自定义变量的访客：表单字段，定义类别，客户ID。
...
区段根据访客的属性或与您站点的交互来设置条件以过滤访客。要在区段内设置条件，可根据访客的特征和/或导航特性来设置规则以过滤访客。要进一步划分访客数据，可以根据每个访客的特定访问和/或页面查看点击来进行过滤。区段生成器提供了一个简单的架构来构建这些子集，并将规则作为嵌套、分层的访客、访问或点击容器应用。
...
细分数据的每个细分都有一个适用的范围。Most breakdowns are based on *Page Views*, however, many valuable segments are based on the *Visit* container, and to a lesser degree the *Visitor* container. 所以，以容器范围为基础了解报表非常关键。
