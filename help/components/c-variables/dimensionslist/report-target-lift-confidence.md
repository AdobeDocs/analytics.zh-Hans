...
Adobe Target 文档包含有关[提升](https://docs.adobe.com/content/help/en/target/using/administer/preferences/estimating-lift-in-revenue.html)和[置信度](https://docs.adobe.com/help/en/target/using/reports/settings/average-lift-bounds-and-confidence-interval.html)的更多信息。

计算提升和置信度：

1. In the **[!UICONTROL Target Activities]** report, click on an activity to bring up its details.
1. 在报告类型下，选择 **[!UICONTROL Lift and Confidence]**。
1. 单击 **[!UICONTROL Show Metrics]** 以添加一个度量。 对于这种类型的报表，您无法添加多个量度，因为它是仅根据一个量度对测试进行评估的最佳实践。多个量度可能只会增加噪声干扰，从而降低测试的信号。
