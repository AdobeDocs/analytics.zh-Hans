...
* **[!UICONTROL 列名称]**：要创建的第一个分类维度的名称。 此字段是分析工作区中使用的维度名称，以及导出分类数据时的列名。可在创建分类集后添加更多列名。
* **[!UICONTROL 类型]**：显示分类类型的单选按钮。 通常使用初级分类；查找分类表示[子分类](../../c-sub-classifications.md)。
* **[!UICONTROL 订阅]** 此分类集适用的报告包和维度。 您可以将多个报表包和维度组合添加到分类集。

![创建分类集](../../assets/classification-set-create.png)

如果给定报表包+变量存在分类集，则将分类添加到架构中。 给定的报表包+变量组合不能属于多个分类集。
