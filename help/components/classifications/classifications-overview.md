...
单个维度可具有多个分类维度。 例如，您可以使用附加产品属性（如产品名称、颜色、大小、描述和 SKU）对[!UICONTROL 产品 ID] 进行分类。 其中每个属性都是一个单独的分类维度，都属于同一个父维度。 使用这些属性扩充您的报告，可提供更深入、更复杂的报告方式。 一旦某个维度包含分类数据，该分类维度便可用于仅包含分类维度项的报告中。 任何不包含分类值的父维度项都会分到[未指定](/help/technotes/unspecified.md)组中
