...
* **电子邮件** 和 **显示广告**:创建分类规则以对各个展示广告型营销活动进行分组，以便您了解展示型营销活动是如何针对电子邮件营销活动执行的。
...
|--- |--- |--- |
| `^(.+)\:(.+)\:(.+)$` | em:JuneSale:20130601 | `$0`:em:JuneSale:20130601  `$1`:em  `$2`:6月销售  `$3`:20130601 |
...
| 匹配字符串a的正则表达式:b:c | `^([^\:]+)\:([^\:]+)\:([^\:]+)$` | a | `$1` |
| 匹配字符串a的正则表达式:b:c | `^([^\:]+)\:([^\:]+)\:([^\:]+)$` | b | `$2` |
| 匹配字符串a的正则表达式:b:c | `^([^\:]+)\:([^\:]+)\:([^\:]+)$` | c | `$3` |
| 匹配字符串a的正则表达式:b:c:d | `^([^\:]+)\:([^\:]+)\:([^\:]+)\:([^\:])$` | d | `$4` |
