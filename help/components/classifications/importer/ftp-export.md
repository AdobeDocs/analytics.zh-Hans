---
title: 通过FTP导出分类数据
description: FTP导出在数据集下载方面提供了更大的灵活性，包括从多个报表包下载数据以及下载大于50,000个数据行的数据集文件
...
* 您可以在定义数据过滤器时使用通配符。使用星号`*`匹配零个或多个字符，使用问号`?`匹配一个字符。 使用`?*`匹配一个或多个字符。
...
这些步骤描述如何使用FTP从Adobe Analytics导出（下载）分类。

1. 创建 FTP 帐户.
1. 转到[!UICONTROL 管理员] > [!UICONTROL 分类导入器]。
1. 单击&#x200B;**[!UICONTROL FTP导入]**&#x200B;选项卡。
1. 配置FTP导出的字段。
1. 单击&#x200B;**[!UICONTROL 导出文件]**。
1. 将数据集保存到您的本地系统中。
...
| [!UICONTROL 选择行数] | 指定要导出多少行数据。<ul><li>选择“**[!UICONTROL 全部]**”可下载所有报表数据。</li><li>如果您要指定要下载的特定行数，请选择“将数据行数限制为”****。</li></ul> |
...
| [!UICONTROL 日期过滤] | （可选）按促销活动数据过滤数据。您只能从活动的促销活动下载数据，或选择在特定日期范围内开始（或结束）的促销活动。 |
| [!UICONTROL 导出数值 2] | 您可以使用导入器将数值2分类导入系统。 数值 2 分类很适合用于针对不同项目（例如营销渠道报表的成本与预算值）随时间变化的变量。 |
| [!UICONTROL FTP 帐户] | 指定您希望 Adobe 从中下载数据文件的 FTP 服务器信息，包括主机名和端口、目标目录的路径、用户名以及密码。 |
| [!UICONTROL 通知] | 指定电子邮件地址来接收有关此 FTP 下载的通知。 |
| [!UICONTROL 编码] | 为数据文件选择字符编码。默认的编码格式为 UTF-8 或 ISO-8859-1，具体情况取决于用来分类而上载的编码。UTF-8 到 UTF-16 可将您的使用 UTF-8 编码的分类转换为 UTF-16 编码。ISO-8859-1 到 UTF-16 可将您的使用 ISO-8859-1 编码的分类转换为 UTF-16 编码。<br>**注意：**&#x200B;如果您选择转换为 UTF-16，那么源编码必须与最初上载的编码相匹配，否则会出现意外结果。我们建议使用不带 BOM 的 UTF-8 格式编码所有已上载的文件。 |
