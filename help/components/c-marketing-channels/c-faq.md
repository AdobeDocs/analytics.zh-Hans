...
* 使用清除流程。 如果“电子邮件”和“附属活动”渠道使用相同的查询字符串参数，但您的电子邮件跟踪代码较少，则可以在定义电子邮件的规则集中指定电子邮件跟踪代码。 然后，使用&#x200B;*`affiliates.`* 分类所有其他跟踪代码。
* 在您的电子邮件系统中，为所有登陆页面 URL 添加一个查询字符串参数，例如 *`&ch=eml`*。 创建一个规则集，用于检测 ch 查询参数是否等于 *`eml`*。 如果该规则集中不包含 *`eml`*，则它是一个附属活动。

## 反向链接域包含的数据比我预期的多。

反向链接域在处理规则列表中可能过高。 它应该是最后（或最后一个）规则集之一，因为处理顺序很重要。

## 我创建了一个与查询字符串参数匹配的规则，但该规则无法运行。

确保在查询字符串参数字段中指定参数名称（通常是字母数字值）。 此外，请确保在运算符之后指定参数值，如电子邮件规则的以下示例所示。

![](assets/example_email.png)

## 为什么我所有的最近联系流量都归因于内部域？

您有一个与内部流量匹配的规则。 请记住，这些规则会处理访客在您的网站上进行的每次点击，不仅是首次访问。 如果您有一个像 *`Page URL exists`* 这样的规则，且没有其他标准，该渠道会与您网站上的每次连续点击相匹配，因为页面 URL 始终存在。

## 我如何调试报告上“未识别渠道”中显示的流量？

规则按顺序处理。 如果没有匹配的特定标准，则点击属于以下三个类别之一：
...
当您的规则没有捕获数据时，或者如果规则配置不正确，报告的[!UICONTROL “未识别渠道”]行中会显示数据。 您可以创建一个名为&#x200B;*Other*&#x200B;的规则集，例如，在处理顺序结束时，该规则集还将标识内部流量。
...
>[!NOTE]
>
>仍可能会有一些渠道流量被列入“未识别渠道”类别。 例如：访客访问网站并将页面加入书签，在同一次访问中通过书签返回页面。 由于这不是访问的第一个页面，因此它不会出现在直接渠道或其他渠道中，因为没有反向链接域。
