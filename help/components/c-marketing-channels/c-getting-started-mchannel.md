...
营销渠道通常用于提供有关访客如何到达您网站的洞察。您可以基于要跟踪哪些渠道以及如何跟踪，自定义营销渠道处理规则。
...
定义您使用的渠道是营销渠道中最重要的一个部分。定义渠道可能需要您的组织中多名人员协作。以下是需要考虑的几个问题：

* 您是否使用付费搜索？
* 您是否使用电子邮件营销活动？您是否使用多个您想单独跟踪的电子邮件营销活动？
* 您是否有附属机构会将流量导向您的网站？是否有您想单独跟踪的附属机构？
* 是否有更适合单独跟踪的外部营销活动？
...
如果渠道是在自动设置中创建的，那么这些渠道中的规则已定义。您可以按照您的需求更改这些规则。

## 自动设置营销渠道 {#run-auto-setup}

营销渠道报表包含一个一次性设置页面，帮助您入门。它提供了多个可用于跟踪的营销渠道。如果您觉得对创建渠道和规则得心应手，可以跳过此设置。不过，Adobe 建议您允许该向导为您创建渠道。自动设置可使您了解规则是如何构建的，还可让您根据自己的需要进行编辑。您可以随时禁用或删除预定义的渠道。
...
1. 单击&#x200B;**[!UICONTROL 编辑设置]** > **[!UICONTROL 营销渠道]** > **[!UICONTROL 营销渠道管理器]**。
   ...
   >[!NOTE]
   >
   >当您访问“管理工具”中的渠道配置应用程序时，会自动显示[!UICONTROL 营销渠道: 自动设置]页面。（请参阅[营销渠道管理器](/help/admin/tools/manage-rs/edit-settings/marketing-channels/c-channels.md)。）如果您的报告包中包含一个或多个营销渠道，此页面就不会显示。除非您选择另一个不含营销渠道的报告包，否则您不能再访问此页面。
