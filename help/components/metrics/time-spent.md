...
| [!UICONTROL 网站平均逗留时间]（秒） | 表示访客与特定维度项目交互所用的总时间，其中每个序列包含一个维度项目。它不仅限于名称所暗示的“站点”平均值。 有关序列的详细信息，请参阅“‘逗留时间’的计算方式”部分。<br>**注意**：由于计算中所用的分母不同，因此，此指标很可能会与维度项目级别的“每次访问逗留时间”有所不同。 | Analysis Workspace、Reports &amp; Analytics（以分钟为单位显示）、Report Builder（以分钟为单位显示） |
...
| [!UICONTROL 页面逗留时间 — 粒度] | 每次点击的总逗留时间，精确至秒。这是一个点击级别的维度，包括页面查看次数和链接事件。尽管其名称有，但并不限于“页面”维度。 | Analysis Workspace |
...
的 [!UICONTROL 页面逗留时间 — 粒度] 维度是点击级别的维度。 按其他维度划分此维度将显示点击持续的秒数，其中也会显示划分维度。在以下示例中，搜索词“分类”与54秒、59秒等的点击时间相关联，这可能表示访客正在花时间阅读针对该搜索词返回的内容。

![](assets/time-spent1.png)

+++

+++哪个量度适用于 [!UICONTROL 页面逗留时间 — 粒度]?
...
+++如何 [!UICONTROL 网站平均逗留时间] 不同 [!UICONTROL 每次访问逗留时间]?
...
+++为什么要总计 [!UICONTROL 网站平均逗留时间] 与父行项目不匹配？

因为 [!UICONTROL 网站平均逗留时间] 取决于维度的未中断序列，而内部报表在计算这些运行时不依赖于外部报表。
...
* “所用秒数”是当前点击的时间戳与下一次点击的时间戳之间的差值。 因此，访问的最后一次点击（和跳出）没有逗留时间。

* “序列”是一组连续的点击，其中给定的变量包含相同的值（无论是设置、扩展还是保留的值）。 例如，prop1“A”有两个序列：点击1和2，点击6。 访问的最后一次点击的值不会开始一个新序列，因为最后一次点击没有逗留时间。网站平均逗留时间使用序列作为分母。

   * 仅出于逗留时间的目的，prop会从页面点击“扩展”到后续链接点击，如上面点击2的prop1所示。 这允许为点击1(“A”)的prop1设置的值累计点击2的逗留时间。
