...
| 量度 | 定义 | 可用位置 |
|---|---|---|
| [[!UICONTROL 所用总秒数]](total-seconds-spent.md) | 表示访客与特定维度项目交互所用的总时间。 包括某个值的实例，以及该值在所有后续点击中的持久性。 对于 prop，也会在后续链接事件中统计逗留时间。 | Analysis Workspace、Report Builder（称为“总逗留时间”）、Data Warehouse |
...
| 维度 | 定义 | 可用位置 |
| --- | --- | --- |
| [[!UICONTROL 每次访问逗留时间 — 粒度]](../dimensions/time-spent-per-visit.md) | 访问期间的总逗留时间会截断到最接近的秒，并应用于属于该访问的每次点击。 这是一个访问级别的维度。 | Analysis Workspace |
| [[!UICONTROL 每次访问逗留时间 — 分段统计]](../dimensions/time-spent-per-visit.md) | 粒度维度分成9个不同的范围。 这是一个访问级别的维度。 范围包括：<ul><li>不到 1 分钟</li><li>1-5 分钟</li><li>5-10 分钟</li><li>10-30 分钟</li><li>30-60 分钟</li><li>1-2 小时</li><li>2-5 小时</li><li>5-10 小时</li><li>10-15 小时</li></ul>**注意**：不会有更高的时段，因为一次访问在活动 12 小时后就会过期。 | Analysis Workspace、Report Builder |
...
| [[!UICONTROL 页面逗留时间 — 分段统计]](../dimensions/time-spent-on-page.md) | 粒度维度分成10个不同的范围；但是，分段维度仅计算页面查看次数（不包括链接事件）。 这是一个点击级别的维度。 范围包括：<ul><li>少于 15 秒</li><li>15-29 秒</li><li>30-59 秒</li><li>1 至 3 分钟</li><li>3 至 5 分钟</li><li>5 至 10 分钟</li><li>10 至 15 分钟</li><li>15 至 20 分钟</li><li>20 至 30 分钟</li><li>超过 30 分钟</li></ul> | Analysis Workspace |
...
* 由于“逗留时间”无从知晓，因此跳出点击和退出点击也会从分母中删除。
...
[[!UICONTROL 页面逗留时间 — 粒度]](../dimensions/time-spent-on-page.md)维度是点击级别的维度。 按其他维度划分此维度将显示点击持续的秒数，其中也会显示划分维度。
在下面的示例中，搜索词“已分类”与54秒、59秒等的点击时间相关联，这可能表示访客正在花时间阅读针对该搜索词返回的内容。
...
任何量度。 此维度将显示发生事件的具体点击的逗留时间。 逗留时间越长，意味着访客在发生事件的页面（点击）上停留的时间越长。
...
* “序列”是一组连续的点击，其中的给定变量包含相同的值（无论是通过设置、扩展还是保留的值）。 例如，prop1“A”包含两个序列：点击1和2以及点击6。 访问的最后一次点击的值不会启动新序列，因为最后一次点击没有逗留时间。 网站平均逗留时间使用分母中的序列作为分母。
