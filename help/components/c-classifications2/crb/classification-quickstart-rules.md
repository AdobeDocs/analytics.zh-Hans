...
* 在“管理工具”中指定分类的 中 [的分类](https://docs.adobe.com/content/help/zh-Hans/analytics/admin/user-product-management/user-groups/groups.html) 、组权限 [!UICONTROL Admin Tools]。
...
* 已对键值进行分类，并且您未选择[覆盖分类](/help/components/c-classifications2/crb/classification-rule-definitions.md)。
   ...
   您可以在[](/help/components/c-classifications2/crb/classification-quickstart-rules.md)添加和激活规则，以及激活 Data Connectors 集成时，覆盖分类。(For data connectors, rules are created by partners in the Dev Center and displayed in the [!UICONTROL Classification Rule Builder].)
...
If a key is matched to multiple rules, and it sets the same classification column shown in the [!UICONTROL Set Classification] column, the last rule is used. 因此，您可能需要将最重要的规则排在规则集的最后。
