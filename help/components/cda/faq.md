...
通过如上所述的“移动设备类型”示例，可查看访客如何在移动设备类型与桌面设备类型之间移动。但是，其中无法区分桌面浏览器与移动浏览器。如果需要此洞察，可创建一个记录该体验是发生在桌面浏览器、移动浏览器还是移动应用程序上的自定义变量（如 prop 或 eVar）。然后可使用该自定义变量代替“移动设备类型”维度创建如上所述的流程图。此方法从稍有不同的视角审视跨设备行为。
