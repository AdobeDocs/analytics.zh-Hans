...
对于基于字段的拼合，标识符自定义变量区分大小写。 如果标识符值与大小写不匹配，“已识别人员”量度的数量可能会高得多。 例如，如果 `bob` 和 `Bob` CDA会将这两个值解释为不同的人，并且这些值应为相同的人。

## 查看标识符prop/eVar时，为什么我看到“未识别人员”量度的值不是零？

当访客在报表窗口中生成经过身份验证的点击和未经身份验证的点击时，通常会发生这种情况。 该访客既属于 [已识别状态](/help/components/dimensions/identified-state.md) 维度，导致将未识别的点击归因到标识符。 此方案在 [重播](replay.md) 运行，具体取决于重播频率和成功率。