...
* Cross-Device Analytics 是按报表包启用的。Adobe 建议在报表包中包含跨设备数据，即，来自多种设备类型（Web、应用程序等）的数据。某些组织将此概念称为“全球”报表包，尽管从地理角度来说，CDA 并不一定包含严格意义上的全球范围。
...
* CDA 当前与“客户属性”不兼容。这两项功能可以在引用同一源报表包的不同虚拟报表包中同时使用。
* CDA 当前与 A4T 不兼容。
