...
跨设备分析是一项功能，可将 Analytics 从以设备为中心的视图转变为以人员为中心的视图。因此，分析师可以了解跨浏览器、设备或应用程序的用户行为。Adobe支持两个总体工作流，将设备数据关联在一起：

* [**基于字段的拼接**](field-based-stitching.md):允许您选择Analytics变量作为虚拟报告套件中跨设备拼接的基础。 使用确定性匹配将设备连接在一起。 Adobe建议对大多数确定性匹配用例使用基于字段的拼接。
* [**设备图**](device-graph.md):CDA与设备图形通信以将设备拼接在一起。 合作图采用确定性匹配和概率匹配。
