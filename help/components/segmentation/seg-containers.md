...
使用 `Page equals Winter Coats` 区段示例，以下是此区段结果的示例，这些结果基于容器数据的应用方式以及数据范围与区段类型的匹配方式。
...
此 `Referring Domain equals aol.com` 以下区段适用于 **页面报表**.
...
由于同一访问内的所有页面查看都分配了相同的反向链接域值，因此会在点击容器级别报告，其中 `Referring Domain equsls 'aol.com'` 返回下表中列出的所有页面。
...
如果在页面报表的访问容器中过滤了此相同条件，则访问中的所有页面 `Referring Domain equals 'aol.com'`为true。 因为反向链接域名的值在访问级别设置，所以页面查看和访问级别的报表是一样的。
...
“页面”报表从“访客”容器中列出了任何访客查看的所有页面，其中 `Referring Domain equals 'aol.com'` 为true。 因此，如果访客拥有 *&#39;aol.com&#39;* 作为反向链接域，在历史记录中的任意时间（在定义的时间段内），访客容器中的所有页面（包括其他访问中的页面查看次数）都会列出。 即使页面与主要条件不匹配，也会列在报表中，因为这些页面包含在访客容器中。 “访客”容器中的所有页面都会列在报表中，即使这些页面以前发生并且不专门满足条件。

在反向链接域报表中， `Referring Domain equals 'aol.com'` 在四个页面查看中为true，但 `Referring Domain equals "weather.com"` 在访客点击的其他页面中为true。 从“访客”容器中，您可以获得一个“aol.com”为true的访客列表。但它也会向您提供反向链接域为“weather.com”的页面，而不是与区段中的初始请求匹配的值。

| 访问1<br/>反向链接域等于“aol.com” | <br/>页面查看次数 |
|----|---:|
| 主页 | 1 |
| 冬季服装 | 1 |
| Winter Coat | 1 |
...
| 访客容器<br/>反向链接域等于“aol.com” | 页面查看次数 |
|----|---:|
