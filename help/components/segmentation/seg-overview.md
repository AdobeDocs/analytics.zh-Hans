...
通过顺序区段，您可以根据网站中的导航和页面查看来识别访客，从而为定义的操作和交互提供一个区段。 顺序区段可帮助您识别访客喜欢和避开的内容。 生成顺序区段时，[!UICONTROL THEN]运算符用于定义和排序访客导航。
...
默认情况下，所有用户均可创建和编辑个人区段。 但是，管理员可以决定谁应具有[权限来创建区段](/help/admin/admin-console/home.md)，并且可以将这些区段分配给特定组。 这些区段可以直接与任何其他Analytics用户共享。

管理员可以编辑任何区段，并与组和组织中的每个人共享区段。 [按角色划分区段权限](/help/components/segmentation/seg-reference/seg-rights.md)

+++

+++ **我可以看到公司的所有区段吗？**

可以，管理员可以在Analysis Workspace用户界面中看到所有区段。
...
如果该区段是[发布到CX Enterprise](/help/components/segmentation/segmentation-workflow/seg-workflow.md)，则不能删除该区段或编辑该区段。 但您可以复制该区段，并对复制版本进行编辑。
