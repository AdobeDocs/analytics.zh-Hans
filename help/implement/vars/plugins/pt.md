...
该 `pt` 插件可执行Analytics变量列表中的函数或方法。 例如，您可以选择性地对多个变 [`clearVars`](../functions/clearvars.md) 量运行该方法，而无需每次手动调用该方法。 其他几个插件依赖于此代码才能正确运行。 如果您不需要同时对多个Analytics变量运行特定函数，或者您未使用任何相关插件，则不需要此插件。
...
以下代码是getQueryParam插件的一部分。  它针对URL的querystring(fullQueryString)中包含的每个键值对运行getParameterValue帮助函数。  另一种方法是，要提取每个键值对，fullQueryString必须用“&amp;”符分隔并拆分。 parameterKey引用插件专门尝试从查询字符串中提取的查询字符串参数
