...
1. 添加直接调用规则，以使用`websiteBot`作为标识符触发Analytics信标。 在此示例中，它使用`s.tl`调用：

   ![websiteBot标识符](assets/websitebot.png)

1. 在直接调用规则中触发Adobe Analytics - Set Variables &amp; Adobe Analytics - Send Beacon操作。  下面的示例中显示了一种执行此操作的方法：
