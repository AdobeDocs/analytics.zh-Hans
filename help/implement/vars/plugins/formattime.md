...
`formatTime` 插件允许您获取任意秒数，并在四舍五入到所需的基准值后以存储段格式提供它们。 如果您希望以秒为单位捕获时间值并将其转化为桶格式（如分钟、天或周），Adobe 建议使用此插件。 如果您不想将基于秒的值存储为时间舍入格式，则不需要使用此插件。
...
* 任何小于 1 分钟的值都会以最接近的 5 秒基准进行四舍五入
* 任何介于 1 分钟和 1 小时之间的值都会以最接近的 1/2 分钟基准进行四舍五入
* 若时间介于 1 小时和 1 天之间，则将以“0.25 小时”为基准四舍五入到最接近的值
* 若时间大于 1 天，则将以“1 天”为基准四舍五入到最接近的值
