...
>[!IMPORTANT]
>
>Adobe 建议不要使用此变量。 请改用[Adobe访客ID服务](https://experienceleague.adobe.com/cn/docs/id-service/using/home)。
...
将此字段分配给包含您的自定义访客 ID 的数据元素。 **不要为所有访客将此字段设置为单个静态值。** 使用根据访客解析并在所有点击中保持不变的数据元素。

## AppMeasurement和Analytics扩展自定义代码编辑器中的s.visitorID

`s.visitorID` 变量是一个字符串，其中包含访客的自定义唯一标识符。 有效值包括最多100字节的字母数字字符。 避免在此变量中使用连字符、空格、下划线或符号。
