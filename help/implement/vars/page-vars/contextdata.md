...
* 有效的上下文数据变量只包含字母数字字符、下划线和句点。 如果您包含其他字符（如连字符），Adobe 不保证处理规则中的数据收集。
...
>[!WARNING]
>
>运行处理规则后，上下文数据变量将被丢弃。 如果您没有启用将值放入变量中的处理规则，则这些数据会永久丢失！
...
4. 创建一个处理规则，将 Analytics 变量设置为上下文数据变量的值。
5. 保存更改。

处理规则在保存后会立即生效。 它们不适用于历史数据。
