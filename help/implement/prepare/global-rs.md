...
* **跨设备分析支持：** CDA 要求一个报表包能从多个位置（如网站和移动设备应用程序）收集数据。如果实施得当，各个设备可以将数据拼合在一起。有关详细信息，请参阅组件用户指南中的[跨设备分析](../../components/cda/cda-home.md)。
...
> [!NOTE] 协调全局报表包实施的工作是一项大工程。为此，Adobe 建议与顾问合作，以尽量避免可能会出现问题和由此产生复杂情况。
