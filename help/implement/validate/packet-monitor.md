...
数据包分析程序允许您查看您的实施发送到 Adobe 数据收集服务器的数据。

与Adobe CX Enterprise Debugger类似，数据包监视工具显示在图像请求中传递的数据参数；而同时，数据包监视工具还提供以下附加功能：
...
Adobe 对图像请求的响应只是一个空白的 1x1 像素透明图像，并且该图像与页面内容无关。 如果在 Adobe 提供的数据包监视器中存在行项目，则无论发生 **[!UICONTROL 200 OK]** 响应还是 **[!UICONTROL NS_BINDING_ABORTED]** 响应，相关数据都会到达 Adobe 服务器。 无需再等待页面。
...
如果外部数据包监视器报告数据收集请求被中止，而不是响应被中止，则这值得关注。 Adobe [!DNL Customer Care] 可帮助进行疑难解答。
