---
title: 映射到 Adobe Analytics 的数据对象变量
...
# 映射到 Adobe Analytics 的数据对象变量

下表显示了 Adobe Experience Platform Edge Network 自动映射到 Adobe Analytics 的数据对象。如果使用这些数据对象字段路径，则无需额外配置即可将数据发送到 Adobe Analytics。
...
此表中的大多数数据对象字段与[所映射的 XDM 字段](xdm-var-mapping.md)一致。如果您同时设置某个给定数据对象字段及其相应的 XDM 字段，则数据对象字段具有优先级。例如，如果字段 `data.__adobe.analytics.events` 存在，它将覆盖所有与事件相关的 XDM 对象字段。
