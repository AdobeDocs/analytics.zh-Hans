...
随着云计算、机器学习和自然语言处理等技术取得最新进展，数字助理日益成为日常生活的一部分。 使用者开始与设备交谈，希望设备能像人一样理解他们所讲的内容并做出响应。 随着这些平台越来越成熟，品牌可以以同样逼真和生动的方式向消费者展示其服务。 例如，消费者可能会问：
...
1. **数字助理**：该设备可以与支持数字助理的服务进行交互。 它是将语音转换为机器可理解意图并解析请求详细信息的位置。 了解用户的意图后，数字助理将请求的意图和详细信息传递给处理请求的应用程序。
1. **“应用程序”**：可以是手机应用程序，也可以是语音应用程序。 应用程序负责响应请求。 它响应数字助理，然后数字助理响应用户。

## 在何处实施Analytics

实施Analytics的最佳位置之一是应用程序中。 应用程序从数字助理处接收意图和详细信息，然后确定如何做出响应。
...
由于数字助理具有对话性，因此他们通常具有会话的概念。 例如：

**消费者：**“确定Google，帮我叫辆出租车”

**Google：**： “没问题，您希望在什么时间搭车呢？”
...
每个数字助理都有算法来检测意图，然后将意图传递给“应用程序”，以便应用程序知道该怎么做。 这些意图简明扼要地呈现了请求。
...
虽然大多数平台不会公开与用户对话的设备，但却会公开所用设备的功能。 例如，音频、屏幕、视频等。此信息非常有用，因为它定义了与用户交互时可以使用的内容类型。 在测量设备功能时，最好按字母顺序将它们串联起来。
