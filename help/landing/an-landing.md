...
* 顶部 Analytics 菜单发生变化：**[!UICONTROL 报告]**&#x200B;顶部菜单不复存在。现在，大多数报告都位于左边栏中的[!UICONTROL 报告]菜单中。
* 左边栏有三个选项卡：[!UICONTROL 项目]、[!UICONTROL 报告]和[!UICONTROL 学习]。我们将在以下各部分中详细讨论其中各个选项卡。
...
| 显示更少/更多内容 | 在不显示和显示横幅之间切换：![](assets/top-banner.png) |
...
| 名称 | 项目的名称。 |
| 固定图标 | **新功能** 您现在可以固定内容，固定后的内容会&#x200B;*始终*&#x200B;显示在列表顶部。 |
...
下面是新菜单及其子菜单。如果找不到特定报告，请执行“搜索页面”以找到它。
...
| 管理员可以为其用户指定此新登陆页面吗？ | 不可以，管理员不能代表用户指定新登陆页面。个人用户必须自行开启切换开关。 |
