...
请注意，要在“自动模式”中配置搜索引擎帐户，您应负责执行以下操作：
...
>[!IMPORTANT]
>
>The `<Advertising Analytics ID>` value (in **bold** below) is generic and **must be replaced with your specific account ID string**. 您可以在帐户设置页面中的“跟踪”部分下，获得特定帐户 ID 字符串。
...
以下显示了需要添加到 Bing 帐户的字符串。您需要将该字符串添加到帐户中使用的所有最终URL后缀。
