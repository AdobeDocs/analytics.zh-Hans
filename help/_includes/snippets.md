...
>[!IMPORTANT]
>
>详细了解 Reports &amp; Analytics [生命周期结束公告](https://www.adobe.com/go/analytics_rnaeol_en)。
...
1. （可选）选择&#x200B;**过滤**&#x200B;图标![，即“数据词典过滤”图标](/help/analyze/analysis-workspace/components/data-dictionary/assets/data-dictionary-filter-icon.png)，然后选择以下任一过滤器选项过滤组件列表：
   ...
   | [!UICONTROL **显示重复项**] | <p>仅显示与选定报表包中其他组件具有相同名称或相同定义的组件。 名称或定义必须完全匹配才能显示为重复项。</p><p>此选项仅适用于管理员。</p><p>**注意：** 对于定义，这包括您创建的组件以及由Adobe提供的组件。 对于名称，当前仅包含您创建的组件，而不包含由Adobe提供的组件。 在将来的版本中，将添加Adobe提供的组件的显示重复名称。</p> |
...
| [!UICONTROL **常常与以下组件一同使用**] | <p>显示最常与您正在查看的组件一起使用的组件。</p><p>在 5 种主要组件类型中，最多显示 5 个组件：量度、计算量度、维度、区段和日期范围。</p><p>此列表基于过去 90 天的数据。只列出您有权限查看的组件。</p><p>管理员可以通过在 [!UICONTROL **始终包含**] 和 [!UICONTROL **始终排除**] 下拉字段。 在您组织用户看到的组件之前，请先应用 **显示全部** 过滤器，以确保您看到未与您共享的任何组件。<!-- Soon we will make it so any fields that an admin doesn't have access to will be greyed out, and then they can enable the Show all filter to make it editable. --></p> |
| [!UICONTROL **如同**] | <p>显示名称与您正在查看的组件类似的组件。</p><p>在 5 种主要组件类型中，最多显示 5 个组件：量度、计算量度、维度、区段和日期范围。</p><p>只列出您有权限查看的组件。</p><p>报表包中的任何重复组件也将显示在此处。 Analytics 管理员应识别并移除所有重复的组件，如[监视数据词典运行状况](/help/analyze/analysis-workspace/components/data-dictionary/monitor-data-dictionary-health.md)中所述。</p><p>管理员可以通过在 [!UICONTROL **始终包含**] 和 [!UICONTROL **始终排除**] 下拉字段。 在您组织用户看到的组件之前，请先应用 **显示全部** 过滤器，以确保您看到未与您共享的任何组件。<!-- Soon we will make it so any fields that an admin doesn't have access to will be greyed out, and then they can enable the Show all filter to make it editable. --></p><p>**注意：** 目前， **类似于** 部分仅包含您创建的组件，而不包含由Adobe提供的组件。 Adobe提供的组件将在将来的版本中添加。</p> |
...
>[!IMPORTANT]
>
>此插件由 Adobe Consulting 团队提供，旨在帮助您从 Adobe Analytics 中获取更多的价值。Adobe 客户关怀团队不提供对此插件的支持，包括安装或疑难解答。如果您需要有关此插件的帮助，请联系贵组织的Adobe帐户团队。 他们可以为您安排与顾问的答疑会，以便您向顾问寻求帮助。
