...
## 旧版Report Builder EOL {#legacy-arb}

>[!WARNING]
>
>旧版Report Builder将于2026年6月终止生命周期。 自2026年7月1日起，将不再提交使用旧版Report Builder计划的工作簿。 <ul><li>您必须在2026年8月12日之前将工作簿从旧版Report Builder迁移到新Report Builder。 有关详细信息，请参阅[转换旧工作簿](/help/analyze/report-builder/convert-workbooks.md)。</li><li>Adobe Analytics 1.4 API的生命周期终止后，将无法再下载旧版Report Builder加载项。</li><li>2026年8月12日之后，如果客户拥有需要迁移的工作簿，则需要联系客户支持团队以获取原始工作簿。 这些工作簿需要按照[转换您的旧工作簿](/help/analyze/report-builder/convert-workbooks.md)中的说明进行迁移。</li></ul> 经过简化的新版 [Report Builder](/help/analyze/report-builder/rb-overview.md) 自 2024 年 10 月 16 日起可用。 macOS、Windows 和 Web 浏览器支持此版 Report Builder，它可与 Adobe Analytics 和 Customer Journey Analytics 一起使用。 Report Builder不再支持计划导出到FTP服务器和Power BI。 在旧版Report Builder中使用这些功能的客户必须在Report Builder中使用[云目标进行导出](/help/analyze/report-builder/report-builder-export.md#schedule-workbooks-by-exporting-to-cloud-destinations)。


## 旧版 Report Builder {#legacy-arb-old}
...
>[!IMPORTANT]
>
>自 **2024 年 1 月 17 日**&#x200B;起，Adobe 终止提供 Reports and Analytics 及其随附的报告和功能。 届时，Reports and Analytics 及其所有报告和计划已停止工作。 支持 Reports &amp; Analytics 的报告、可视化图表和底层技术不再满足 Adobe 的技术标准。 在 Analysis Workspace 中提供了 Reports &amp; Analytics 的大部分功能。 相关信息请参阅[使用模板](/help/analyze/analysis-workspace/templates/use-templates.md)。
> 
...
>[!AVAILABILITY]
>
>本节中介绍的功能仅适用于已具有相应功能许可证的现有客户。 此功能不再作为附加组件提供给现有客户或新客户。
>

## 视频演示 Customer Journey Analytics {#videocja}

此视频演示了使用 Customer Journey 的功能。 然而，此功能在 Adobe Analytics 中同样可用。 请注意 Adobe Analytics 与 Customer Journey Analytics 之间的术语差异（例如&#x200B;*访问次数*&#x200B;与&#x200B;*会话*&#x200B;的不同）。
...
| ![算法](/help/assets/icons/AttributeAlgorithmic.svg) | 算法 | 使用统计技巧，动态确定所选量度的最佳点数分配。 归因算法基于合作博弈理论中的 Harsanyi Dividend 算法。 Harsanyi Dividend 算法是对“沙普利值”解决方案（以诺贝尔经济学奖获得者罗伊德·沙普利 (Lloyd Shapley) 命名）的推广形式，用于为对结果具有不同贡献的各参与者分配功能值（点数）。<br>从一个较高的层面来看，归因通过参与者联盟的方法计算，剩余值必须公平地分配给这些参与者。 每个联盟的剩余值分配都根据每个子联盟（或先前参与的维度项）先前产生的剩余值通过循环计算得出。 有关更多详细信息，请参阅John Harsanyi和Lloyd Shapley的原文：<br>Shapley， Lloyd S. (1953). A value for n-person games. *对游戏理论的贡献，2(28)*，307-317.<br>Harsanyi，John C. (1963). A simplified bargaining model for the n-person cooperative game. *International Economic Review 4(2)*, 194-220. |
...
* **访问**：从“访问”容器的范围查看转化情况。 如果选择了&#x200B;**[!UICONTROL 访问]**，[归因回顾窗口](#atribution-lookback-window)就会自动设置为&#x200B;**[!UICONTROL 报告窗口]**，且无法更改。
* **访客**：从“访客”容器的范围查看转化情况。
...
| 模型 | 容器 | 回顾时间范围 | 说明 |
|---|---|---|---|
| 首次接触点 | 访问 | 报告窗口 | 归因仅考虑第三次访问。 在电子邮件与展示广告之间，电子邮件是首次接触点，因此电子邮件在 50 美元的购买中获得 100% 的点数。 |
| 首次接触 | 访客 | 30 天 | 归因会考虑所有三次访问。 付费搜索是首次接触点，因此它在 50 美元的购买中获得 100% 的点数。 |
| 线性 | 访问 | 报告窗口 | 点数在电子邮件和展示广告之间进行分配。 这两个渠道各自获得贡献 25 美元的点数。 |
...
| 功能 | 历程画布 | 流失 | 流量 |
|---------|----------|---------|---------|
| **页面预定义序列** | 是</br>预定义分析和探索性分析相结合。 使用路径上的预定义节点时会使用最终路径（只要访客最终从一个预定义节点移动到另一个预定义节点，就会被计算在内）。 也可以显示紧接着（不是最终）的下一个节点。 | 是</br>路径可以是最终路径，也可以被限制到下一个接触点 | 否 |
| **页面探索序列（临时分析）** | 是</br>预定义分析和探索性分析相结合。 使用路径上的预定义节点时会使用最终路径（只要访客最终从一个预定义节点移动到另一个预定义节点，就会被计算在内）。 也可以显示紧接着（不是最终）的下一个节点。 | 有限</br>您可以单击右键，在自由格式表中查看立即流失。 | 是</br>仅探索性分析。 始终在节点之间的一个维度实例内。 这意味着每个节点都显示路径上紧邻的（而非最终的）下一个接触点。 |
| **显示人们离开（流失）和继续通过（流过）的位置** | 是</br>为预定义历程和探索性历程显示 | 是</br>显示预定义历程 | 是</br>显示探索性历程 |
| **线性历程** | 是 | 是 | 否 |
| **具有多个入口点和路径的非线性历程** | 是 | 否 | 是 |
| **主要量度** | 任何量度，包括计算量度 | 仅会话或人员 | 仅发生次数（路径视图） |
| **辅助量度** | 是<p>任何量度，包括计算量度</p> | 否 | 否 |
| **节点或接触点中的组件支持** | 量度、维度项、筛选条件和日期范围。 | 量度、维度项、筛选条件和日期范围。 | 仅维度项（起始和结束接触点除外） |
| **比较筛选条件** | 否 | 是<p>在同一份报告中，对两种不同的过滤器进行并排比较。</p> | 否 |
| **拖放组件互动** | 是 | 是 | 否 |
...
## 报告包筛选部分 {#reportsuitefiltersection}

| 报告包 | 描述 |
|---|---|
| ![报告包](/help/assets/filter-reportsuite.png){width="300"} | 在&#x200B;**[!UICONTROL 报告包]**&#x200B;部分可以筛选报告包。 <ul><li>您可以![搜索](/help/assets/icons/Search.svg) *搜索报告包*，搜索可以用于进行筛选的报告包。</li><li>您可以选择多个报告包。 可用的报告包取决于在筛选面板的其他部分中所做的选择。</li><li>这些数字表明：<ul><li>**(2)**：所选报告包的数量（如果选择了一个或多个报告包）。</li><li>**3︎⃣**：当前筛选条件产生的结果项可用的报告包数量。</li><li>4︎⃣：与特定的报告包相关联的项数。</li></ul></li></ul> |
...
>[!WARNING]
>
>分类导入器将于 **2026 年 8 月 31 日**&#x200B;弃用。 请改用[分类集](/help/components/classifications/sets/overview.md)体验，以确保继续使用相关功能。
>



## 分类规则生成器弃用 {#classification-rulebuilder-deprecation}

>[!WARNING]
>
>分类规则生成器将于 **2027 年 2 月 28 日**&#x200B;弃用。 届时将提供[分类集规则](/help/components/classifications/sets/manage/rules.md)体验，请改用后者以确保继续使用相关功能。
>
