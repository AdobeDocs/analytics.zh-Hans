---
description: 无需用户交互，重定向即可将浏览器指向新位置。 它们在Web浏览器（客户端重定向）或Web服务器（服务器端重定向）中执行。
...
无需用户交互，重定向即可将浏览器指向新位置。 它们在Web浏览器（客户端重定向）或Web服务器（服务器端重定向）中执行。

## 重定向和别名 {#aliases}

无需用户交互，重定向即可将浏览器指向新位置。 它们在Web浏览器（客户端重定向）或Web服务器（服务器端重定向）中执行。

由于重定向不需要用户进行任何交互，因此重定向通常会在用户不知晓的情况下执行。 表明已发生重定向的唯一因素是浏览器的地址栏。 地址栏显示的URL与浏览器最初请求的链接不同。

尽管只有两种类型的重定向，但它们可以通过多种方式实施。 例如，可能发生客户端重定向，因为用户将其浏览器指向的网页包含脚本或特殊HTML代码，可将浏览器重定向到其他URL。 可能发生服务器端重定向，原因是页面包含服务器端脚本或Web服务器已配置为将用户指向其他URL。

## Analytics 和重定向 {#aa-redirects}

[!DNL Analytics] 会从浏览器收集一些数据，并且依赖于某些特定的浏览器属性。 其中两个属性，“反向链接URL”（或“反向链接”）和“当前URL”可通过服务器端重定向进行更改。 由于浏览器知道已请求一个URL，但返回了不同的URL，因此它清除了反向链接URL。 结果，反向链接 URL 为空，[!DNL Analytics] 可能会报告页面不存在反向链接。
...
重定向可能会导致浏览器将真正的引荐URL遮蔽起来。 请考虑下列情景︰
...
由于浏览器会删除反向链接URL，因此您必须配置相应的机制，以便处理重定向（例如，Web服务器、服务器端代码、客户端代码）并传递原始反向链接信息。 如果您还想记录别名链接URL，则必须将此链接传递到最终登陆页面。 使用&#x200B;*`s_pageURL`*&#x200B;变量覆盖当前URL。
...
这些变量将在[CX Enterprise Debugger](https://experienceleague.adobe.com/docs/debugger/using/experience-cloud-debugger.html)中表示为以下参数。

<table id="table_5F3B987D4D514CA283F7B9F52EBC2301"> 
 <thead> 
  <tr> 
   <th class="entry"> </th> 
   <th class="entry"> <b>URL或查询字符串值</b> </th> 
   <th class="entry"> DigitalPulse Debugger中显示的<b>值</b> </th> 
  </tr> 
 </thead>
 <tbody> 
  <tr> 
