...
数据控制者确定如何解析发出请求的“数据主体”的身份标识。 可以考虑部署 Adobe 的数据隐私 ID 检索标记。 您的开发团队通过使用我们的数据隐私 ID 检索标记来捕获用户 ID (Cookie ID) 来节省时间。 然后，他们可以使用我们的数据隐私API将这些用户ID发送到Adobe CX Enterprise中的相关解决方案，以进行数据隐私请求处理。 数据隐私 API 可以在多个 Adobe 解决方案中支持范围广泛的客户 ID。

如果数据主体连同标识符（自定义变量 - prop 或 eVar）一起提交请求，则 Adobe Analytics 会扫描为给定标识符收集的整个保留历史记录。 有关如何配置存储在 Analytics prop 或 eVar 中的自定义 ID 的详细信息，请参阅 [Analytics 关于命名空间的文档](/help/admin/tools/privacy-labeling/namespaces.md)。

+++

+++ **Adobe Analytics 数据管理怎样协助处理数据隐私请求？**

“数据管理”是 Adobe Analytics 中的一个新工具，它赋与数据控制者在其 Analytics 数据中应用数据控制和分类的能力。 这款新工具允许 Adobe 客户自定义其数据隐私数据访问和数据删除请求的处理方式。 在“数据治理”控制台中，管理员可以定义应应用于 Adobe Analytics 中各种数据列的所需设置。 一旦定义了这些标签，Adobe 将根据客户所需的标签设置，执行并处理任何下游访问或删除请求。 数据控制者有责任进行审查，并就这些标签设置与其法律代表进行协商。
...
通常情况下，在向公众发布之前，Analytics 客户会设置一些测试报告包来验证功能。 预生产网站或应用程序将数据发送到这些测试/开发/QA 报告包中，以评估在将实际流量发送到生产报告包之前代码发布时的运行情况。

但是，在正常配置下，在将请求应用到生产报告包之前，不能先在这些测试报告包上对 GPDR 请求的处理进行测试。 这是因为数据隐私请求会自动应用于CX Enterprise组织中的所有报表包，通常也是您公司的所有报表包。
...
作为数据控制者，您有责任在收集与数据主体相关的数据（可能包括 Adobe Analytics 数据）之前获得他们的明确同意，并且有责任在您的网站上实施[选择退出机制](https://www.adobe.com/cn/privacy/opt-out.html#customeruse)。 这样，您的数据主体就可以选择退出将来的Adobe CX Enterprise数据收集。
...
客户可通过致电客户关怀请求在 25 个月之内删除他们的数据。 此外，客户还可以通过付费，将数据保留期限延长到 25 个月以后。 可按每次额外延长 1 年的增量提供延期，最多可额外延长 8 年（总计 10 年）。 这些延期将需要更新合同条款并支付额外费用。
...
另外，还需要考虑员工已经下载了包含个人数据的 Adobe Analytics 报告的情况。 如果收到与数据隐私相关的删除请求，且该请求涉及报告中存在的某个 ID，则这些报告可能需要更新或删除。 客户应当与自己公司的法律顾问合作，确定保留期以及应用于这些类型文档的隐私和安全要求。

+++

+++ **我们意外将一些不应该收集的数据发送到了 Adobe Analytics。 是否可以使用数据隐私 API 来清理此类数据？**

提供[数据 Privacy Service API](https://developer.adobe.com/experience-platform-apis/references/privacy-service/) 是为了帮助您执行对时间敏感的数据隐私请求。 Adobe 不支持将此 API 用于其他目的，因为这样可能会影响 Adobe 及时处理其他 Adobe 客户发起的高优先级数据隐私请求的能力。

我们建议您不要将数据隐私 API 用于其他目的，例如清除大量访客意外提交的数据。 您还应该了解，对于任何因提交数据隐私删除请求后而删除其点击（已更新或匿名）的访客，其状态信息都将会进行重置。 该访客下次返回您的网站时，将会成为新访客。 所有eVar归因都将重新开始，访问次数、反向链接、访问的第一个页面等信息也都将重新统计。对于要清除数据字段的情况，这种副作用不尽如人意，因此突显了数据隐私API不适合此用途的一个原因。

请联系您的 Adobe 帐户团队，与我们的工程架构师咨询团队协调，以进一步审查并评估移除任何 PII 或数据问题所需的工作量。
...
您还应该了解，对于任何因提交数据隐私删除请求后而删除其点击（已更新或匿名）的访客，其状态信息都将会进行重置。 该访客下次返回您的网站时，将会成为新访客。 所有eVar归因都将重新开始，访问次数、反向链接、访问的第一个页面等信息也都将重新统计。对于要清除数据字段的情况，这种副作用不尽如人意，因此突显了数据隐私API不适合此用途的一个原因。
...
* CX Enterprise数据隐私[保护包](https://landing.adobe.com/dam/uploads/2018/in/adobe_gdpr_carepackage.pdf)
