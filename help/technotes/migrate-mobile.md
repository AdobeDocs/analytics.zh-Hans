...
处理规则用于将上下文数据变量的值迁移到 prop 和 eVar。 例如，您可以将“search-term”上下文数据变量的值放入 Commerce Variable eVar 的值中，并在每次点击时覆盖该值。 如果没有处理规则，上下文数据变量将毫无意义，并且不会在 Analytics 中填充任何报表。

本文档还会向您演示如何在 Analysis Workspace 完成移动设备使用情况报告。

## 迁移处理规则

如果您正在利用 Mobile Services 的免费功能（例如处理规则和使用情况报告功能），则可以无缝迁移到 Analytics UI（处理规则 UI 或 Analysis Workspace）来完成这些功能。 对于生命周期量度，或者在 AA 处理规则 UI 中设置的规则，无需进行任何迁移。 生命周期量度是现成可用的量度，当您首次在应用程序中实施 Mobile SDK 时，会自动收集这些量度。

但是，如果您在 Mobile Services UI 中设置了任何其他处理规则（生命周期量度除外），则应迁移这些处理规则，以便在失去对 Mobile Services 的访问权限后，您可以在 Analytics 中编辑/删除这些规则。
...
1. 在CX Enterprise中，切换到Adobe Analytics并确保您位于在Mobile Services中查看的同一移动设备报表包中。
1. 转到&#x200B;**[!UICONTROL 管理]** > **[!UICONTROL 报表包]** > **[!UICONTROL 编辑设置]** > **[!UICONTROL 常规]** > **[!UICONTROL 处理规则]**。
1. 单击&#x200B;**[!UICONTROL 添加规则]**。
1. 忽略条件，然后继续添加 Mobile Services 中存在的相同上下文变量。
...
除了移动量度和维度之外（如果为 Mobile Services 启用了报告包），Analysis Workspace 将包含多个有助于分析的 Mobile 项目模板：
...
* 推送消息传送和应用程序内消息传送在 Adobe Campaign Standard 和 Adobe Campaign Classic（仅限推送）中可用。 但是，用于定位的底层数据集是不同的。 我们建议您与 Adobe 帐户团队合作来确定消息传送数据的迁移选项。
