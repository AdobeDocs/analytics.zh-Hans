...
如果数据主体提交了一个请求和一个标识符（自定义变量 – prop 或 eVar），Adobe Analytics 则会扫描为给定标识符收集的完全保留的数据历史记录。有关如何配置存储在 Analytics prop 或 eVar 中的自定义 ID 的详细信息，请参阅 [Analytics 关于命名空间的文档](/help/technotes/c-data-governance/data-labeling/gdpr-namespaces.md)。
