...
* Adobe Analytics 使用&#x200B;**[!UICONTROL 低阈值]**&#x200B;和&#x200B;**[!UICONTROL 高阈值]**&#x200B;这两个阈值确定每个月在报告中显示哪些独特值。这些阈值可由Adobe不时调整。 当前的阈值限制是：
   * **[!UICONTROL 低阈值]**：该月有2,000,000个唯一值。
   * **[!UICONTROL 高阈值]**：该月有2,100,000个唯一值。
* 如果变量在给定月份未达到低阈值，则报告不受影响。
* 当变量达到低阈值时，数据开始被存储在标记为[!UICONTROL 低流量]的维度项下。 超出此阈值的每个值都遵循以下逻辑：
   * 如果某个值已在报表中，则正常添加到该值。
   * 如果报表中尚未出现某个值，则将其最初添加到[!UICONTROL 低流量]维度项。
   * 如果在[!UICONTROL 低流量]下存储的值收到大量流量（通常在一天内出现两位数的实例），则将其识别为自己的维度项。 在流量流入之前收集的实例保持在[!UICONTROL 低流量]下。 维度项目开始在报告中出现的确切时间店有许多依赖关系，如为报告包处理数据的服务器的数量以及每个维度项实例之间的时长。
...
请注意，对于具有在一个月内重复多次的维度项目的变量，低流量逻辑最适用。 如果变量的维度项目在每次点击时几乎或完全唯一，则变量的唯一值数量会快速达到两个阈值，并且当月的所有后续维度项目最后都会进入低流量存储段。
