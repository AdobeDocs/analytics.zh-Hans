...
用户输入的“提交时间”或“时间”可指定报表应开始处理的时间，而不是实际提交的时间。 提交报表的时间主要取决于处理时间。 处理复杂和大型报表比处理简单的报表耗时更长。 例如，如果报表需要15分钟才能处理，则实际提交时间将至少比最初指定的“提交时间”或“时间”晚15分钟。

此外，还有许多其他因素可能会进一步延迟实际提交报表的时间：

* **同时运行多个相同类型的计划**（例如，许多功能板等）会导致系统过载。计划系统仅允许同时运行少数(5-10)个任何类型的报表。如果同时计划了超过5-10个报表，则有些报表将需要排队等待其他报表完成，然后才能开始处理。 通过交错安排一天或一小时中处理公司报表的时间，而不是同时安排，可以缓解此问题。
