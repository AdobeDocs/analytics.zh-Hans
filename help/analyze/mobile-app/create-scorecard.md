...
例如，您可以通过将适当的维度（如本示例中的 **[!DNL Marketing Channel]**）拖放到图块上来将它添加到&#x200B;**[!UICONTROL 独特访客]**&#x200B;指标。Dimension划分显示在 [!UICONTROL 钻取] 详细幻灯片中特定的部分 **[!UICONTROL 属性]**. 可向每个图块中添加多个维度。
...
在记分卡生成器中单击图块时，右侧边栏会显示与该图块及其详细信息幻灯片关联的属性和特征。 在此边栏中，您可以提供 **标题** 对于图块，或者通过应用区段来配置图块。

![“属性”图块](assets/properties_tile.png)

## 查看详细幻灯片 {#view-detail-slides}

单击图块时，动态弹出窗口会显示详细幻灯片在应用程序中向执行用户显示的方式。 您可以添加维度来根据您的特定需求划分数据。 如果未应用任何维度，则划分维度将 **小时** 或 **天数**，具体取决于默认日期范围。
...
添加到详细幻灯片的每个维度都将显示在应用程序详细幻灯片视图的下拉列表中。 之后，执行用户可以从下拉列表所列的选项中进行选择。
