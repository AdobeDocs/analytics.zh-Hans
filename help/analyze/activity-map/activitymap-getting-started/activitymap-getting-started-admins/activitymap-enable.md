...
## 步骤 1. 更新实施代码 {#section_5D1586289DF2489289B1B6C1C80C300D}

Activity Map模块是AppMeasurement.js和Web SDK(版本2.15.0或更高版本)的一部分。
实例化后，AppMeasurement库或Web SDK将加载Activity Map模块。

>[!NOTE]
>
>Activity Map数据无法收集，除非您更新 **AppMeasurement** **版本1.6** 或 **Web SDK** **版本2.15.0** 或更高


1. 根据您使用的是AppMeasurement还是Web SDK，下载最新的Javascript库。

   - **AppMeasurement** 代码(AppMeasurement_Javascript-1.6.zip)，方法是转到  **[!UICONTROL Analytics]** > **[!UICONTROL 管理员]** > **[!UICONTROL 所有管理员]** > **[!UICONTROL 代码管理器]** 和 [实施it](https://experienceleague.adobe.com/docs/analytics/implementation/js/overview.html?lang=zh-Hans).
   ...
   - **Web SDK** 代码(alloy.js)。 请参阅 [安装SDK — 选项2:安装预建的独立版本](https://experienceleague.adobe.com/docs/experience-platform/edge/fundamentals/installing-the-sdk.html?lang=zh-Hans#option-2%3A-installing-the-prebuilt-standalone-version) 以了解更多信息。 确保您使用的是版本2.15或更高版本。

      请参阅 [跟踪链接](https://experienceleague.adobe.com/docs/experience-platform/edge/data-collection/track-links.html) 有关如何实施链接跟踪以及如何通过捕获活动映射来启用活动映射的信息 `region` 的HTML元素。

      >[!NOTE]
      >
      >当客户从一个页面导航到下一个页面时，启用Web SDK的链接跟踪当前会发送链接事件。 这与AppMeasurement的工作方式不同，可能会导致向Adobe发送额外的可计费点击。
