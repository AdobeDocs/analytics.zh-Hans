...
| **[!UICONTROL 报表包]** | 您可以访问的报表包列表不再局限于网页标签中定义的报表包。现在，您可以将选定的报表包（对应于页面上的标签之一）替换为其他报表包。不需要将此新报表包链接到页面上的标签。如果您在“Activity Map设置”中更改选定的报表包，则保存过程会刷新所有受影响的Analytics报表。<br>**重要信息**: [!UICONTROL 虚拟报] 表包与实时模式 [!UICONTROL 不兼容]，仅与标准 [!UICONTROL 模式兼容]。如果您在标准报表包的[!UICONTROL 实时模式]中，但在此对话框中选择了[!UICONTROL 虚拟报表包]，则当您在此处单击&#x200B;**[!UICONTROL 确定]**&#x200B;后，将显示标准模式。 此外，日历控件已重新初始化，以匹配报表包的日历类型（公历、零售、自定义……）。 |
...
| **[!UICONTROL 渐变透明度]** | 为渐变叠加图选择透明度。此设置不会影响[!UICONTROL Bubble]叠加图。 |
...
| 设置 | 描述 |
| --- | --- |
| **[!UICONTROL 动态数据过滤]** | 此下拉菜单允许您显示<ul><li>（默认）页面上的所有链接</li><li>页面上排名最前（最高）或最低（最低）的链接数量，其中#可以选择1、10、50或100。</li></ul> |
...
| 设置 | 描述 |
|---|---|
| **[!UICONTROL 显示排名最前的]** | 要将&#x200B;**[!UICONTROL 获胜方]**&#x200B;或&#x200B;**[!UICONTROL 失败方]**（或同时显示两者）显示为叠加图，请选择链接数量。 |
