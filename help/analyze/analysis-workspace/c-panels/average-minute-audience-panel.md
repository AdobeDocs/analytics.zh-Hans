...
| 将某个区段（或任何其他组件）拖至此处 | 与其他面板一样，此设置根据您创建的区段筛选您的选择。 这是查看特定平台、直播或其他常见媒体区段的一个好方法。 |
...
| 节目、季、集 | 选择“节目、季、集”将在下拉菜单中显示您的可用节目，从中可使用搜索（或通过从左列拖放节目名称）进行过滤。 可在这里结束选择以查看您的各季节目，也可按个别的季过滤，然后按个别的集过滤。此设置显示这些节目、季和集在所选时段的数据。 |
