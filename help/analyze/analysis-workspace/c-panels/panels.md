...
将任意区段从左边栏拖放到面板拖放区域以开始筛选面板。 重复此过程以向面板中添加其他筛选器。 过滤器在面板顶部并排显示。
...
利用下拉区段，您可以通过可控方式与数据交互。 例如，您可以为移动设备类型添加下拉区段，以便可以按平板电脑、手机或台式机来划分面板。

下拉区段也可用于将多个项目合并为一个。 例如，如果您有一个项目的多个版本，每个版本具有不同的“国家/地区”区段，您可以将所有版本合并为一个项目并添加“国家/地区”下拉区段。
...
* 对于使用维度项目的下拉区段，从左边栏中选择单个维度并将其放到面板拖放区域中 **同时保持`[Shift]`**. 这将创建一个下拉区段，其中包含与该维度关联的所有维度项。
  ...
  或者，如果您希望下拉区段仅包含与维度关联的特定维度项目，请在左边栏中单击所需维度旁边的右箭头图标。 此操作会公开所有可用的维度项目。使用以下方式从此列表中选择多个维度项 `[Shift + Click]` 或 `[Ctrl + Click]`，然后将它们拖放到面板拖放区域中，同时保持 `[Shift]`.
...
用户可以通过以下任意方式使用下拉菜单来筛选面板：

* 通过从下拉菜单中选择筛选器，将单个筛选器应用到面板。

* 通过从下拉菜单中选择多个过滤器，将多个过滤器应用于面板。 将筛选面板以包含任何选定的过滤器。
