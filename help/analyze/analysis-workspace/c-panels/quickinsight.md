...
>[!IMPORTANT]
>
>**[!UICONTROL Quick Insights目前]** 只进行有限的测试，并非所有Adobe Analytics客户都能使用。
...
| [!UICONTROL 可视化图表] | 工作区 [优惠多种可视化](/help/analyze/analysis-workspace/visualizations/freeform-analysis-visualizations.md) ，以构建数据的可视化表示形式，如条形图、圆环图、直方图、直线图、地图、散点图等。 |
| [!UICONTROL 划分] | 细分是一种按其他维度来划分维度的方法。 在我们的示例中，您可以按移动设备划分美国州，以获取每个州的移动设备访问次数，或按移动设备类型、地区、内部活动等划分移动设备。 |
...
1. 添加至少一个维和一个度量后，将为您创建以下内容：
   ...
   * 随附的可视化，本例为条 [形图](/help/analyze/analysis-workspace/visualizations/bar.md)。 生成的可视化基于您添加到表中的数据类型。 您可以通过单击“条”旁边的下拉箭头来更改可视化 **[!UICONTROL 类型]**。
