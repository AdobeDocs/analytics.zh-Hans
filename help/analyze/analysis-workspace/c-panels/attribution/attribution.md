...
| ![算法](assets/algorithmic.png) | [算法](/help/analyze/analysis-workspace/c-panels/attribution/algorithmic.md) | (注： 算法归因目前处于有限测试中。) 使用统计技术动态确定所选指标的最佳信用分配。 | 有助于避免在为业务选择正确的归因模型时进行猜测或试探。 |
...
* **自定义回顾窗口：** (注： 自定义回顾窗口当前处于受限测试中。) 允许您将归因窗口扩展到超出报告日期范围（最多90天）。 在报告期内的每次转换时，都会评估自定义回顾窗口。 例如，对于2月20日发生的转换，10天的回顾窗口将评估归因模型中2月10日到20日的所有维接触点。
...
>[!NOTE] 如果区段抑制包含您的度量的点击，则这些度量实例不会归因到任何维。 但是，类似的报表过滤器只会隐藏一些维度值，而不会影响根据归因模型处理的度量。 因此，具有类似定义的区段和过滤器有时可能会返回较低的区段值。
