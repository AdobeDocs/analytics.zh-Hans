...
* **** 自定义回顾窗口：允许您将归因窗口扩展到超出报告日期范围（最多90天）。 自定义回顾窗口将在报告期内的每次转换中进行评估。 例如，对于2月20日发生的转换，10天的回顾窗口将评估归因模型中从2月10日到20日的所有维接触点。
...
* 使用 **第一次触** 摸和自定义回顾窗口 ****（7天），归因只查看最近两次访问。 在此报告窗口中，社交媒体链接是第一个链接，因此它可以获得50美元购买的100%信用。
...
* 使用 **线性****和七天的自定义回顾窗口** ，信用在社交、电子邮件和显示之间进行划分。 每个渠道都可获得此购买的12.50美元积分。 付费搜索被排除，因为它不在指定的回顾窗口内发生。
...
* 使用&#x200B;**时间衰减归因模型**&#x200B;和&#x200B;**访客回顾窗口**，则付费搜索、社交、电子邮件和展示广告均可获得点数。使用默认的 7 天半衰期：
   * 显示接触点与转化之间的间隔为 0 天。`2`<sup>`(-0/7)`</sup> `= 1`
   * 电子邮件接触点与转化之间的间隔为 0 天。`2`<sup>`(-0/7)`</sup> `= 1`
   * 社交接触点与转化之间的间隔为 6 天。`2`<sup>`(-6/7)`</sup> `= 0.552`
   * 付费搜索接触点与转化之间的间隔为 9 天。`2`<sup>`(-9/7)`</sup> `= 0.41`
   * 将这些值标准化处理之后得到以下结果：
