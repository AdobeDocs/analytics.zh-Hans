...
观看演示视频的![VideoCheckedOut](/help/assets/icons/VideoCheckedOut.svg) [多个报表包](https://experienceleague.adobe.com/zh-hans/docs/analytics-learn/tutorials/analysis-workspace/using-panels/multiple-report-suites-in-analysis-workspace){target="_blank"}。
...
>[!TIP]
>
>您可以拖放到与活动面板位于同一报告包中的任何面板。 通过拖放到同一报告包的非活动面板，该面板将变为活动面板。
>
...
1. 将组件（量度、维度、区段、日期范围）拖放到面板中。 确保面板具有特定于其报告包的数据和可视化图表。


   >[!NOTE]
   >
   >有时，在加载项目（或切换到报告包）时，如果该项目中的所有组件并非全部包含在报告包中，则会显示一个横幅。 系统将会列出缺少的组件。 按照[这些说明](/help/admin/admin-console/permissions/product-profile.md)，设置所需量度/维度的权限。
   >
...
1. 将面板更改为其他报告包，并注意组件标签（当前为活动的报告包）和列出的组件如何根据新的报告包进行更新。

1. 使用键盘快捷键（拖动时按 `shift`）将非活动面板转换为活动面板。

1. （可选）您还可以转到其他 Analytics 组件生成器，并确保它们现在显示一个报告包标签，指示
