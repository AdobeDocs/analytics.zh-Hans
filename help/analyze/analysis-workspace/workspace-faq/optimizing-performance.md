...
| 因素 | 定义 | 优化 |
| --- | --- | --- |
| 请求数量 | 向Adobe发出的请求总数，这些请求用于检索项目中显示的数据。 查询包括表格的排名请求、异常检测、迷你图、左边栏中显示的组件等。不包括折叠面板和可视化图表。该准则为 100。 | 可采取以下方法来尽可能地简化项目：将数据拆分至多个用于特定目的或利益相关方组的项目中。运用标记将项目整理到不同的主题中，并运用[直接链接](https://experienceleague.adobe.com/docs/analytics/analyze/analysis-workspace/curate-share/shareable-links.html)来创建内部目录，以便利益相关方能够更轻松地找到所需内容。 |
...
| 因素 | 定义 | 优化 |
| --- | --- | --- |
| [!UICONTROL **平均请求时间**] | 从启动请求时到请求完成时所需的时间。 <p>在上面的[请求处理](#request-processing-diagram)图中，请求时间表示从&#x200B;**Analysis Workspace请求启动**&#x200B;到&#x200B;**Analysis Workspace请求完成**&#x200B;的完整进程。</p> |  |
...
| [!UICONTROL **平均查找时间**] | 由于Analysis Workspace只存储任何区段中使用的任何字符串的哈希，因此每次处理项目时，都会执行&#x200B;**查找**，以将哈希与相应的值相匹配。 <p>此过程可能会占用大量资源，具体取决于可能与哈希匹配的值的数量。 </p><p>在上面的[请求处理](#request-processing-diagram)图中，查找时间表示在&#x200B;**查找**&#x200B;阶段（在&#x200B;**请求引擎处理**&#x200B;阶段）。</p> | 如果此处请求速度减慢，可能是因为项目中的字符串区段过多，或者字符串中带有过于通用的值，潜在匹配项过多。 |
| [!UICONTROL **平均队列时间**] | 处理请求之前在队列中等待的总时间。<p>在上面的[请求处理](#request-processing-diagram)图中，队列时间表示在&#x200B;**请求引擎队列**&#x200B;阶段和&#x200B;**服务器队列**&#x200B;阶段。</p> | 如果此处请求速度减慢，可能是由于在您的组织中同时运行的请求过多。 尝试在非高峰时间运行请求。 |
| [!UICONTROL **平均服务器处理时间**] | 处理请求所用的平均时间。<p>在上面的[请求处理](#request-processing-diagram)图中，平均服务器处理时间表示在&#x200B;**服务器队列**&#x200B;阶段和&#x200B;**服务器处理**&#x200B;阶段。 | 如果此处请求的速度减慢，则项目可能会有过长的日期范围或复杂的可视化图表。 请尝试缩短项目日期范围，以减少处理时间。 |
...
| [!UICONTROL **月边界**] | 请求中包含的月数。 更多的月份边界会增加请求的复杂性。 | 如果此处请求速度减慢，可能是因为项目中的月份边界过大。 尝试减少月数。 |
| [!UICONTROL **列**] | 请求中的量度和细分的数量。 更多的列会增加请求的复杂性。 | 如果此处请求速度减慢，可能是因为项目中的列太多。 请尝试减少列数。 |
| [!UICONTROL **区段**] | 应用于请求的区段数。 更多区段会增加请求的复杂性。 | 如果此处请求速度减慢，可能是因为项目中的区段太多。 请尝试减少区段数。 |
