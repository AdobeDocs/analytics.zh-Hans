...
| 并发查询 | 贵组织同时请求Adobe的查询数。 每个组织至少有权获得5个并发查询。 | 如果报表需要较长时间，通常是由于报表与其他报表处于队列中所致。 这意味着您的组织正在尝试针对特定报表包运行多个并发请求。 查询可以来自API请求、报表UI(Analysis Workspace、Reports &amp; Analytics、Report Builder等)、计划项目、计划报表、计划警报以及发出报表请求的并发用户。 | 将报表包的请求和计划较为均匀地分布在一天当中。此外，如果可能，请将请求切换到非高峰时间。 星期一上午、星期二上午和每月的第一天都是报告高峰期。 |
