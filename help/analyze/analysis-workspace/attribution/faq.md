...
某些基于访问的度量，如[条目](/help/components/metrics/entries.md)或[跳出率](/help/components/metrics/bounce-rate.md)，可以将数据归因到报告窗口开始日期范围之前的某个时段。 这种情况是由于使用回顾窗口的归因模型，该窗口确定归因应该考虑多远来衡量指标。 最常见的情况是访问时间跨过午夜。 例如：

1. 用户于9月7日晚上11点55分访问您的主页。
1. 他们访问了几页，最后一页发生在9月8日凌晨12点05分。
1. 一周后，您将运行日期范围为9月8日至9月14日的每日趋势报告。

基于点击的指标，如[页面视图](/help/components/metrics/page-views.md)将产生预期输出；从9月8日到9月14日，数据每天都在趋势化。 但是，基于访问的指标也将显示9月7日的上述访问。 访问的归因条目发生在9月7日，默认情况下，回顾窗口为9月1日至9月31日。

在此示例中，弹出率在9月7日始终显示0%。 此度量定义为`Bounces divided by Entries`，基于点击的度量除以基于访问的度量。 弹回由单个图像请求组成，因此它们不能跨多天。9月7日的任何弹回都发生在报告窗口之外，这将导致当天的0%保证弹回率。 其他基于点击的指标也将在此报告中显示9月7日的0，因为这些点击也不在报告窗口内。

再看一个类似的例子。 以下示例与上面的示例之间唯一的区别是日期：

1. 用户于8月31日晚上11点55分访问您的主页。
1. 他们访问了几页，最后一页发生在9月1日凌晨12点05分。
1. 一周后，您将运行日期范围为9月1日至9月7日的每日趋势报告。

在此示例中，“条目”和“弹出率”不显示8月31日起的数据。 回顾窗口和报告窗口均为9月1日开始，因此数据不能从8月31日起归因。
