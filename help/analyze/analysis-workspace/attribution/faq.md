...
“无”行项目是一个统括项，表示在回顾窗口内没有任何接触点的情况下发生的所有转化。 要减少归因于“无”行项目的转化的次数，请尝试使用回顾期较长的“自定义回顾窗口”。
...
1. 用户于9月7日晚上11:55访问您的主页。
1. 他们访问了一些页面，其中最后一页的访问时间为9月8日凌晨12:05。
...
1. 用户于8月31日晚上11:55访问您的主页。
1. 他们访问了一些页面，其中最后一页的访问时间为9月1日凌晨12:05。
1. 一周后，您获得了一份每日趋势报告，该报告的日期范围是 9 月 1 日至 9 月 7 日。

在本例中，“进入次数”和“跳出率”不会显示 8 月 31 日起的数据。 由于回顾窗口和报告窗口均从 9 月 1 日开始，因此无法将 8 月 31 日起的数据归因。
...
不会。 归因模型会使用报告时处理，这项功能仅在 Analysis Workspace 中可用。 有关更多信息，请参阅[报表时间处理](/help/components/vrs/vrs-report-time-processing.md)。
...
由于记录不按顺序排列，应用持久性的预期值可能会影响所提供的交易 ID 时间戳与原始交易之间的时间长度。
...
元数据维度（如匹配类型和关键词）可与归因结合使用。 但是，量度（包括展示次数、成本、点击量、平均排名和平均质量分数）使用摘要级别的数据源，因此不兼容。
...
Adobe Analytics 中的某些维度在单次点击中可包含多个值。 常见示例包括列表变量和产品变量。
...
归因始终在分段之前运行，并且进行分段之后才能应用报表过滤器。 此概念也适用于使用区段的虚拟报告包。

例如，如果创建一个应用了“展示广告点击”区段的虚拟报告包，则您可能会在使用某些归因模型的表中看到其他渠道。

![仅显示虚拟报表包](assets/vrs-aiq-example.png)

>[!NOTE]
>
>如果区段抑制了包含您的量度的点击，则这些量度实例不会归因到任何维度。 但是，类似的报表过滤器只会隐藏一些维度项目，而不会影响根据归因模型处理的量度。 因此，区段返回的值可能会比具有类似定义的过滤器返回的值少。
