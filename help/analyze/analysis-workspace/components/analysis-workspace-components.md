...
1. 在搜尋欄位中，開始輸入您要在專案中使用的元件名稱。
   ...
   元件型別可由顏色和圖示識別。 **Dimension** ![Dimension圖示](assets/dimension-icon.png) 是橙色， **區段** ![「區段」圖示](assets/segment-icon.png) 是藍色的， **日期範圍** ![日期範圍圖示](assets/date-range-icon.png) 是紫色的，並且 **量度** ![量度圖示](assets/default-metric-icon.png) 是綠色的。 Adobe圖示 ![Adobe圖示](assets/default-calc-metric-icon.png) 表示計算量度範本或區段範本，以及計算器圖示 ![計算器圖示](assets/calculated-metric-icon-created.png) 表示貴組織中Analytics管理員建立的計算量度。
