...
Analysis Workspace中的日期比较允许您采用包含日期范围的任何列来创建常见的日期比较，例如：年同比、季度同比，或月同比。

## 比较时段

分析需要上下文，并且以前的时间段通常会提供该上下文。 例如，问题&#x200B;*与去年的这个时候相比，您现在的表现好或坏多少？* 对于了解您的业务至关重要。 日期对比功能会自动包含一个&#x200B;*差异*&#x200B;列，用于显示与指定时段相比的百分比变化。
...
例如，您对上一周（截至2024年10月5日）和前一周执行了逐日比较。 默认情况下，左列从9月22日开始，右列从9月29日开始。

![日期未对齐](assets/not-align-dates.png)

您可以在自由格式表格可视化的[设置](/help/analyze/analysis-workspace/visualizations/freeform-table/freeform-table.md#settings-1)中启用&#x200B;**[!UICONTROL 对齐日期，使各列的日期从同一行开始]**，从而实现日期对齐。 第一列用于此对齐。
...
See ![VideoCheckedOut](/help/assets/icons/VideoCheckedOut.svg) [Date comparison](https://experienceleague.adobe.com/zh-hans/docs/analytics-learn/tutorials/analysis-workspace/calendar-and-date-ranges/using-date-ranges-and-comparisons-in-analysis-workspace){target="_blank"} for a demo video.
