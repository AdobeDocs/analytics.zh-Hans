...
您可以从 [!UICONTROL Components] 菜单将标准量度、计算量度或计算量度模板拖放到表中：
...
或者，您也可以单击 **[!UICONTROL Components]** > **[!UICONTROL New Metric]**。此操作可将您转到[计算量度生成器](https://docs.adobe.com/content/help/zh-Hans/analytics/components/calculated-metrics/cm-overview.html)，您可以从现有量度创建自定义量度。

为了更便于快速创建计算量度，**[!UICONTROL Create metric from selection]** 已添加到自由格式表的列右键单击菜单中。在选定一个或多个标头列单元格时，会显示该选项。
