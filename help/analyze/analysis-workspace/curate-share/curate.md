...
要在报表包级别应用策划，以便同时将策划应用于多个项目，您可以 [在虚拟报表包中策划组件](https://experienceleague.adobe.com/docs/analytics/components/virtual-report-suites/vrs-components.html?lang=zh-Hans).

>[!NOTE]
> 始终先应用虚拟报表包策划，然后再应用项目策划。 这意味着即使您的策划项目包含某些组件，但是如果策划的虚拟报表包不包含这些组件，系统也会将其过滤掉。

## “显示所有组件”选项

在应用策划后的项目或虚拟报表包中，收件人将看到以下选项 **[!UICONTROL 显示全部]** 组件左边栏中的。 [!UICONTROL 显示所有组件]选项将显示不同的组件集，具体取决于：
...
|---|---|---|---|
| 策划的虚拟报表包 | 所有非策划的虚拟报表包组件 | 此角色拥有或已与其共享的非策划虚拟报表包组件 | 此角色拥有或已与其共享的非策划虚拟报表包组件 |
| 策划的项目 | 所有非策划的项目组件 | 所有非策划的项目组件 | 此角色拥有或与其共享的非策划的项目组件 |
| 策划虚拟报表包中的策划项目 | 所有非策划的组件，如下所示 **[!UICONTROL 非策划的项目组件]** 和 **[!UICONTROL 非策划的虚拟报表包组件]** | 此角色拥有或已与其共享的所有非策划项目组件以及非策划虚拟报表包组件 | 此角色拥有或已与其共享的非策划虚拟报表包和项目组件 |
