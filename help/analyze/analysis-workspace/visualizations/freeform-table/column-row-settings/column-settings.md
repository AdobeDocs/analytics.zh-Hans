...
| 异常 | 确定此列中的值是否要运行异常检测。有关更多信息，请参阅 [在Analysis Workspace中查看异常](/help/analyze/analysis-workspace/virtual-analyst/c-anomaly-detection/view-anomalies.md). |
...
条件格式将格式应用于由您定义的上限、中点和下限。除非选择了“自定义”限制，否则对于各种划分，还将在自由格式表中自动应用条件格式（例如颜色）。
