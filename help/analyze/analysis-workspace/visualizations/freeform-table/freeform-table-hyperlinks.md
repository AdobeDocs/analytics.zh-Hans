...
* 包含具有 URL 值的细分的维度项（例如，具有“页面 URL”维度细分的“页面名称”维度）。
...
观看演示视频的![VideoCheckedOut](/help/assets/icons/VideoCheckedOut.svg) [维度](https://experienceleague.adobe.com/zh-hans/docs/analytics-learn/tutorials/analysis-workspace/tips-and-tricks/create-hyperlinks-in-freeform-tables){target="_blank"}的超链接。
...
1. 选择&#x200B;[!UICONTROL **创建**]。
   ...
   查看自由格式表的用户会看到带有超链接的维度项。 单击维度项时，用户将被带到单独的浏览器选项卡中的超链接页面。
...
1. 在 Analysis Workspace 中的自由格式表中，执行以下操作之一：
   ...
   * **移除维度列中所有维度项的超链接：**
   ...
   如果您选择了单个维度项，则会从单个维度项中移除超链接。 或者，如果您选择了维度列标题中的维度名称，则会从所有维度项中移除超链接。
