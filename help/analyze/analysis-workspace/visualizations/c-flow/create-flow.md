...
1. 在项目中添加一个空白面板，然后单击左边栏中的可视化图标。
   ...
   采用中“将可视化图表添加到面板”部分中所述的任何方式添加可视化图表 [可视化概述](/help/analyze/analysis-workspace/visualizations/freeform-analysis-visualizations.md).
...
流量配置的摘要显示在图的顶部。图中的路径厚度与其活动量成正比，活动量大的路径比活动量小的路径厚度大。

要进一步深入了解数据，您有几个选项：

* 流量图是交互式图表。将鼠标置于该图上可更改显示的详细信息。

* 在图中的某个节点上选择时，将显示该节点的详细信息。 再次选择该节点以将其折叠。

  ![节点详细信息](assets/node-details.png)

* 您可以筛选列以仅显示某些结果，例如包括和排除、指定条件等。

* 选择左侧的加号(+)展开一列。

* 使用下面介绍的右键单击选项进一步自定义输出。

* 选择配置摘要旁边的铅笔图标以进一步编辑流量或使用不同的选项重建流量。

* 您还可以将流量图导出为项目的 .CSV 文件的一部分并对其进行进一步分析，方法是转到&#x200B;**[!UICONTROL 项目]** > **[!UICONTROL 下载 CSV]**。

## 正在筛选

当您将鼠标悬停在每列上方时，会显示出一个过滤器。通过选择过滤器，您将获得与当前自由格式表中存在的相同的过滤器对话框。 此过滤器的工作原理与它在自由格式表格中的工作原理相同。
...
* 要删除筛选器，请选择筛选器图标以打开筛选器菜单。 删除应用的任何筛选器，然后选择 **[!UICONTROL 保存]**. 流量应返回到其先前的未过滤状态。
