...
Analysis Workspace中的预建报表可快速分析最常见的报表方案。 以下是您可以使用预建报表回答的一些问题示例：
...
以下信息介绍了如何从访问和使用预建报表。 [!UICONTROL 报表] Analysis Workspace选项卡。
...
1. 在搜索字段中，开始键入要查找的报表的名称，然后从报表列表中选择该报表。 您也可以按照 prop、eVar 和事件编号来搜索报表列表。<!-- still true? -->

   或

   选择要查看的报表类别，然后从报表列表中选择该报表。

   >[!TIP]
   >
   >   要使用箭头键浏览菜单，请按正斜杠键(/)，然后按向下箭头键。 按Enter加载所选报表。

有关可用报告的列表，请参见 [可用报表](#available-reports) 部分。
