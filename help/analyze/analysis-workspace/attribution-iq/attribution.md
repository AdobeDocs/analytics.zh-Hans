...
>[!IMPORTANT]
>
>Adobe Analytics Ultimate、Prime、Select和Foundation SKU上的所有客户均可使用归因IQ。
...
* 第一次访问的/$10将在搜索、显示、社交和电子邮件之间平均分配，每个用户收到/$2.50。
* 在第二次访问时，搜索和电子邮件将分别收到/$5转换的一半，因此电子邮件和搜索将分别收到另一次/$2.50。
* 最后，在最后一次访问中，电子邮件将收到/$2转换的所有积分。

在&#x200B;**访客回顾窗口**&#x200B;中，会同时考虑所有转化，但由于存在多次转化，因此其计算会稍微复杂一些。

* 第一次/$10转换将在搜索、显示、社交和电子邮件中平均分配。
* 第二个/$5转化将在该访问中出现的渠道以及上次访问中的先前渠道之间进行拆分：搜索=(2/6)* /$5 = /$1.67，显示=(1/6)* /$5 = /$0.83，社交=(1/6)* /$5 = /$0.83，电子邮件=(2/6)* /$5 = /$1.67。
* 最后，访客的最后一次转化将跨所有渠道进行：搜索=(2/7)* /$2 = /$0.57，显示=(1/7)* /$2 = /$0.29，社交=(1/7)* /$2 = /$0.29，电子邮件=(3/7)* /$2 = /$0.86。
...
>[!NOTE]
>由于归因模型的命中级别分配，因此，由于每个值接收到包含该报表的每个行项目的总信用百分比，因此报表的每行项目的总和可能不等于总计。

## 带细分的归因

归因始终在分段之前运行，分段在应用报告过滤器之前运行。 这些原则也适用于应用了区段的虚拟报表包(VRS)。

例如，如果创建应用了“显示点击”区段的VRS，则在启用归因后，您可以在表格中看到返回的其他营销渠道。 这是因为归因在未细分的数据集中运行。
