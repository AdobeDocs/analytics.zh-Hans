...
您可以计划工作簿、指定高级传递选项、指定收件人，并查看计划历史记录。 高级传递选项允许您配置要在特定时间发送或定期发送的工作簿。 您还可以指定发送工作簿时要使用的文件格式。

例如，您可以安排工作簿立即提交或者按定期计划提交，还可以指定 [!DNL Advanced Delivery Options] 中的文件格式。 上传工作簿时，文件大小不能超过 5 MB。
...
>[!NOTE]
>
>用户输入的“提交时间”/“时间”指定了应开始处理工作簿的时间，而不是实际提交工作簿的时间。 工作簿实际送达的时间主要取决于处理所需的时间（处理复杂和大型工作簿比处理简单的工作簿耗时更长）。 例如，如果需要 15 分钟才能处理工作簿，则实际提交时间将至少比最初指定的“提交时间”/“时间”晚 15 分钟。
...
| 字段 | 描述 |
|--- |--- |
| 选择报表 | 工作簿的名称。 对于新的计划报表，此字段使用活动工作簿名称填充。 |
| 选择 | 显示“选择报告”页面。 您可以从服务器（先前计划的所有工作簿都存储在其中）选择报表，也可以从本地计算机选择报表。 如果从本地驱动器选择 .xls 格式的工作簿，系统会将文件转换为 .xlsx。 作为该转换的一部分，文件将在Excel中打开并激活。 如果为计划报表选择的工作簿与当前在Excel中打开的工作簿具有相同的文件名，则系统会选择本地文件，而不是先前上载的文件。 如果从计划存储库中选择报表，则会在服务器上创建工作簿的副本，其文件名将更新为1。 新创建的计划报表使用复制的工作簿。 |
| 自定义 | 允许您自定义日期格式。 |
| 至 | 显示您的Outlook通讯簿（如果适用）。 |
| 发送至：电子邮件 | 工作簿的电子邮件收件人。 |
| Power BI | 有关详细信息，请参阅[将工作簿发布到Microsoft Power BI](/help/analyze/legacy-report-builder/c-publish-power-bi/integration-power-bi.md)。 |
| 主题 | 用户定义的描述。 |
...
|--- |--- |
| **计划选项卡** |  |
| 提交时间 | 允许您安排工作簿立即发送或稍后发送。 一天中的时间相对于计算机上指定的时区。 |
| 周期性模式 | 基于您的选择发送工作簿。 |
...
| 文件目标 | 指定电子邮件或FTP。 页面上的选项会根据您的选择而更改。 对于FTP，必须确保主机在外部可用。 |
| 文件内容语言 | 指定要用于随函的语言。 您可以选择中文（简体或繁体）、德语、法语、日语、朝鲜语、巴西葡萄牙语或西班牙语。 |
| **发布选项选项卡** |  |
| 发布到 Power BI | <ul><li>将工作簿发布到 Power BI</li><li>将所有 Report Builder 请求发布为 Power BI 数据集</li><li>作为 Power BI 数据集表发布所有带格式的表</li></ul> |
| 为此Power BI报表添加标签 | 标签设置详细信息 |
