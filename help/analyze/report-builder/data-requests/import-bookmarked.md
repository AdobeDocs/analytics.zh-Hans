---
description: 现在，所有标为书签的报表和功能板报表都作为维度列在“请求向导”的第 1 步中，并且可以导入为 Report Builder 请求。
title: 导入加入书签的报表和功能板缩图报表
topic: Report builder
uuid: 0fdbdb2e-5db7-4f64-b571-23482ba3606d
translation-type: tm+mt
source-git-commit: dabaf6247695bc4f3d9bfe668f3ccfca12a52269

---


# 导入加入书签的报表和功能板缩图报表

现在，所有标为书签的报表和功能板报表都作为维度列在“请求向导”的第 1 步中，并且可以导入为 Report Builder 请求。

当您选择已添加书签的报表时，请求向导将填充定义此已添加书签的报表的所有维和度量。 日期范围、粒度和选定区段也会根据选定的书签进行更新。

这就是请求向导第1步显示仪表板及其缩图报表的方式：

![](assets/import_dashboard_reportlet.png)

单击或时， **[!UICONTROL Retrieve your Dashboards]** 将检 **[!UICONTROL Retrieve your Bookmarks]**&#x200B;索您的现有仪表板和／或书签数据并将其粘贴到工作表中。

>[!NOTE] 在 Report Builder 中，用户可用的功能板和书签列表是有限的，此外，应用于您在向导的第 1 步中选择的报表包的功能板和书签列表也是有限的。与此相反，在市场营销 Reports &amp; Analytics 中，您可以访问所有可用的书签和功能板，无论这些功能板和书签使用哪些报表包。

>[!NOTE] 只导入数据，因此，如果书签包含图表，或者如果功能板缩图报表仅由图表组成，则只会导入用于填充图表的数据。

通过导入仪表板缩图报表（或书签）创建请求后，该请求将关联到缩图报表（或书签）的主维。 因此，如果您编辑请求，树视图将不再选择仪表板报表树视图节点（或书签节点）:而是选择其主维。

导入的书签工具会将报表包、选定的区段、维度和选定量度正确设置为“报告与分析”书签中显示的相同参数。

>[!IMPORTANT]
>
>日期范围将会设置为相同的日期范围，但是会作为静态日期范围 - 即使此日期范围在 Reports &amp; Analytics 书签中为滚动日期范围也是如此。

