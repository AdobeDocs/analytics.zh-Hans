...
要映射的区域和单元格数量因您选择的量度、粒度、日期范围和设置的过滤器而不同。 例如，如果您选择 [!UICONTROL 网站量度] > [!UICONTROL 流量报表]，设置 [!UICONTROL 周] 粒度，并设置日期范围 [!UICONTROL 最近2周]，则系统会提示您映射三个单元格(使用 [!UICONTROL 自定义布局]) [!UICONTROL 请求向导：第2步]. 该请求会检索第一周的数据和第二周的数据，其中每个数据点值等于页面查看的值。 第 3 个单元格充当行标题，您可以使用“[!UICONTROL 格式选项]”对其进行配置。
...
如果错误地选择映射到已具有活动映射的单元格，则区域选取器图标旁边的文本框中不会显示任何单元格引用。 当您单击 [!UICONTROL 确定]，Report Builder显示错误， *选定范围与另一个请求的范围相交。 请更改您的选择。*
