...
| 注意事项 | 添加或更新日期 | 描述 |
| ----------- | ---------- | ---------- |
| **Adobe Experience Platform Edge Network的完整IP模糊处理** | 2023 年 9 月 27 日 | Adobe计划在2023年10月为来自Edge Network的点击更新IP模糊处理。 去年4月，边缘网络添加了对IP地址进行模糊处理的功能。 当时，由于Analytics处理来自Edge Network的点击的方式，Adobe Analytics仅支持IP的部分模糊处理。 当客户为Edge Network选择完全模糊处理时，Analytics仅收到部分模糊处理的IP。 实施此更改后，Analytics会收到完全模糊处理的IP。 |
...
| 注意事项 | 添加或更新日期 | 描述 |
| ----------- | ---------- | ---------- |
| **支持接收已模糊处理的 IP 地址** | 2023 年 9 月 27 日 | Adobe计划为来自Adobe Experience Platform Edge Network的点击更新IP模糊处理。 2023年4月，边缘网络添加了对IP地址进行模糊处理的功能。 当时，由于Analytics处理来自Edge Network的点击的方式，Adobe Analytics仅支持IP的部分模糊处理。 当客户为Edge Network选择完全模糊处理时，Analytics仅收到部分模糊处理的IP。 在执行此更改后，Analytics现在会收到完全模糊处理的IP。 |
...
| **Adobe Experience Platform Edge Network的IP模糊处理** | Edge Network的Adobe计划支持对直接发送到Adobe Experience Platform的数据进行IP模糊处理。 这项更改使直接将数据发送到Platform以用于Customer Journey Analytics或其他Platform解决方案的客户受益。 IP 地址模糊处理是在数据流级别配置的。它支持删除最后一个八位字节或整个 IP 地址。<p>**注释**：模糊处理不适用于发送到 Adobe Analytics 的数据。Analytics 继续获取完整的 IP。IP 处理继续在 Analytics 中单独完成。将来，Adobe计划允许在Edge Network级别对Analytics数据进行模糊处理。 | 不适用 | Adobe Experience Platform 发行版（2023 年 4 月 26 日） |
