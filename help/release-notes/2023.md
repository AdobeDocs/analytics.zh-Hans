...
| **更新了的命名约定 [浏览器](../components/dimensions/browser.md) 维度** | 2023 年 1 月 17 日 | 从适用于Chrome、Edge和Firefox浏览器的版本110开始，公司名称不再显示在浏览器之前。 如果您具有基于浏览器名称的区段定义，这可能会影响报表。 例如，从110版开始，包含“浏览器包含‘Google’”定义的区段将不再标识Chrome浏览器。 此外，从Chrome 110开始，移动设备版本和非移动设备版本的Chrome标记为相同。 以前，移动设备版本和非移动设备版本会单独进行标记。 您可以使用移动设备分段和划分来比较移动设备流量与非移动设备流量。 |
