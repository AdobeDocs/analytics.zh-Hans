...
| 注意事项 | 添加或更新日期 | 描述 |
| ----------- | ---------- | ---------- |
| **设备查找流程更新** | 2023 年 3 月 2 日 | 作为引入对客户端提示的支持的一部分，Adobe 现在与 DeviceAtlas 合作进行所有设备查找。以前与 DeviceAtlas 的合作仅包括移动设备查找。作为该引入的一部分，某些桌面操作系统不当地标有文本 `"mobile"`。例如，维度项显示 `"Mobile OS X 10.15.7"`，而不是 `"OS X 10.15.7 "`。Adobe 计划在 4 月份的版本中更正这些名称。以可追溯的方式更新 Analytics 和 Customer Journey Analytics 报告。对于[!UICONTROL 数据馈送]，如果存储并使用在查找中引用的操作系统 ID，则更改可追溯。但是，如果执行查找并将实际操作系统值存储在您的数据中，则更改无法追溯。 |
