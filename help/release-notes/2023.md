...
| 注意事项 | 添加或更新日期 | 描述 |
| ----------- | ---------- | ---------- |
| **Adobe Experience Platform Edge Network 的完整 IP 模糊处理** | 2023 年 9 月 27 日 | Adobe 计划于 2023 年 10 月更新来自 Edge Network 的点击进行 IP 模糊处理。 去年 4 月，Edge Network 添加了模糊处理 IP 地址的功能。 那时，由于 Analytics 处理来自 Edge Network 点击的方式，Adobe Analytics 仅支持对 IP 进行部分模糊处理。 当客户为 Edge Network 选择完全模糊处理时，Analytics 仅接收到已部分模糊处理的 IP。 实施此更改后，Analytics 会接收到已完全模糊处理的 IP。 |
...
| 注意事项 | 添加或更新日期 | 描述 |
| ----------- | ---------- | ---------- |
| **支持接收已模糊处理的 IP 地址** | 2023 年 9 月 27 日 | Adobe 计划对来自 Adobe Experience Platform Edge Network 的点击进行 IP 模糊处理更新。 2023 年 4 月，Edge Network 添加了模糊处理 IP 地址的功能。 那时，由于 Analytics 处理来自 Edge Network 点击的方式，Adobe Analytics 仅支持对 IP 进行部分模糊处理。 当客户为 Edge Network 选择完全模糊处理时，Analytics 仅接收到已部分模糊处理的 IP。 做出此更改后，Analytics 现在可以接收完全模糊处理的 IP。 |
...
| **深层链接（移动应用程序）** | 使用户可发送记分卡的链接，而这些链接将引导用户直接进入应用程序中的记分卡项目。 这样使得可更轻松地从不太熟悉技术的受众共享项目和提高参与度。 [了解详情](/help/analyze/mobile-app/create-scorecard.md#share-scorecards-using-a-shareable-link) | 不适用 | 2023 年 5 月 17 日 |
...
| **Web SDK 对 Activity Map 的部分支持** | 从 Web SDK 版本 2.15.0 开始，我们开始在启用链接跟踪时填充 Activity Map 数据。 这样使得 Web SDK 用户如果用在 Analytics 中配置的 Web SDK 和 Activity Map 启用了链接跟踪，则可获得 Activity Map 报告。<p>当客户从一个页面导航到下一个页面时，用 Web SDK 启用链接跟踪当前将发送链接事件。 这与 AppMeasurement 的工作方式不同，并且可能会导致将额外的可计费点击数发送到 Adobe。 | 不适用 | 2023 年 3 月 31 日 |
...
| 注意事项 | 添加或更新日期 | 描述 |
| ----------- | ---------- | ---------- |
| **设备查找流程更新** | 2023 年 3 月 2 日 | 作为引入对客户端提示的支持的一部分，Adobe 现在与 DeviceAtlas 合作进行所有设备查找。 以前与 DeviceAtlas 的合作仅包括移动设备查找。 作为该引入的一部分，某些桌面操作系统不当地赋予了文本标签 `"mobile"`。 例如，维度项显示 `"Mobile OS X 10.15.7"`，而不是 `"OS X 10.15.7 "`。 Adobe 计划在 4 月份的版本中更正这些名称。 以可追溯的方式更新 Analytics 和 Customer Journey Analytics 报告。 对于[!UICONTROL 数据馈送]，如果存储并使用在查找中引用的操作系统 ID，则更改可追溯。 但是，如果执行查找并将实际操作系统值存储在您的数据中，则更改无法追溯。 |
...
| **增强的用户偏好设置** | 您现在可以在用户级别配置其他偏好设置（在[!UICONTROL 组件]>[!UICONTROL 偏好设置]中）。 当您设置用户偏好设置时，您的选择会跨越您的项目、表格和可视化图表。 偏好设置页面现在包含以下新选项卡，每个选项卡包含许多新的配置选项：<ul><li>自由格式表</li><li>可视化图表</li></ul>此外，现在可以在&#x200B;**[!UICONTROL 整体]**&#x200B;和&#x200B;**[!UICONTROL 项目]**&#x200B;选项卡上使用更多首选项。<p>以前，其中许多偏好设置只能针对单个项目、表格和可视化图表进行配置。 [了解详情](/help/analyze/analysis-workspace/user-preferences.md) | 不适用 | 2023 年 1 月 11 日 |
...
| **改进了 IP 到地理位置映射** | 2023 年 1 月 4 日 | 我们的 IP 查找供应商 Digital Element 正在升级到新的改进数据集 (NetAcuity Pulse)，用于 IP 到地理位置映射。 Adobe Analytics 原计划于 2022 年 10 月采用此新数据集，实际于 **2023 年 1 月 11 日**&#x200B;采用。 新数据库比以前的版本更准确。 当采用新数据库时，一些 IP 到地理位置的映射将发生变化/改善。<p>所有 Adobe Analytics 工具（Analysis Workspace、Reports &amp; Analytics、Reporting API、Data Warehouse、LiveStream、Analytics Data Feeds 等）都会自动利用新改进的映射。 数据馈送中的数据格式不会发生变化。 通过 Analytics Source Connector 提供的 Customer Journey Analytics 数据也会自动利用新映射。 |
