...
| **管理组件时可用的新列** | 在管理组件时，现在有以下新列可用：<ul><li>**使用位置**：此列在计算量度管理器和区段管理器中可用。</li><li>**上次使用时间**：此列在计算量度管理器、区段管理器和警报管理器中可用。</li></ul>此信息可帮助您确定某个组件是否对组织中的用户有用、在何处使用该组件以及是否需要删除或修改该组件。可使用数据词典配合此信息帮助您跟踪并更好地了解如何在您的组织中使用组件。 | 2023年9月20日 | 2023 年 10 月 4 日 |
...
| 注意事项 | 添加或更新日期 | 描述 |
| ----------- | ---------- | ---------- |
| **支持接收已模糊处理的 IP 地址** | 2023 年 9 月 27 日 | 针对来自 Experience Edge 的命中的 IP 模糊处理将更新。2023 年 4 月，Experience Edge 添加了 IP 地址模糊处理功能。那时，由于 Analytics 处理来自 Experience Edge 的命中的方式，Adobe Analytics 仅支持对 IP 进行部分模糊处理。当客户为 Experience Edge 选择完全模糊处理时，Analytics 仅收到已部分模糊处理的 IP。利用此更改，Analytics 现在将收到已完全模糊处理的 IP。 |
