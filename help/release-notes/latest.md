...
| **新 AppMeasurement 变量** | 变量 `doubleEncodeLinkParameters` 适应边缘情况，其中实施会在链接跟踪变量中编码多字节字符。大多数实施无需定义此变量。[了解详情](../implement/vars/config-vars/doubleencodelinkparameters.md) |  | 2023 年 6 月 7 日 |
| **数据馈送导出的安全目标** | 现在可以将数据馈送发送到以下云存储目标：<ul><li>Amazon S3</li><li>Azure RBAC</li><li>Azure SAS</li><li>Google Cloud 平台</li></ul>不再推荐以前可用的目标（FTP、SFTP、S3 和 Azure Blob）。[了解详情](https://experienceleague.adobe.com/docs/analytics/export/analytics-data-feed/create-feed.html?lang=zh-Hans) | 2023 年 6 月 12 日 | 2023 年 7 月 15 日 |
...
| 功能 | 描述 | [开始推出](releases.md) | [正式发布](releases.md) |
| ----------- | ---------- | ------- | ---- |
| **从自由格式表中删除包含动态维度的行** | 现在可在 Analysis Workspace 的自由格式表中使用 x 图标快速地删除包含动态维度的特定行。这样做时自动应用“始终排除项目”筛选规则。<p>而以前只有在“过滤器”对话框中手动创建规则才能删除包含动态维度的行。[了解详情](/help/analyze/analysis-workspace/visualizations/freeform-table/filter-and-sort.md)</p> | 不适用 | 2023 年 5 月 17 日 |
...
| **动态下拉筛选器** | 我们正在引入一个新类型的下拉筛选器，它根据面板报告范围内的数据和其他下拉筛选器中的值来确定可用的值。您可以通过按住 [!UICONTROL Shift] 并将维度拖入面板的拖放区域来使用动态下拉筛选器。通过按住 [!UICONTROL Shift] 并将一组维度项拖入面板的拖放区域来使静态下拉筛选器保持不变。[了解详情](/help/analyze/analysis-workspace/c-panels/panels.md) |  | 2023 年 6 月 14 日 |
| **更新后的 Analytics 学习页面** | Adobe Analytics 登录页面上的“学习”选项卡现在包含以下增强功能：<ul><li>改进的设计，可在单个页面上通过改进后的导航来显示更多学习内容</li><li>能够根据体验级别（初学者、中级和高级）个性化学习内容</li></ul>[了解详情](/help/analyze/landing.md) |  | 2023 年 6 月 30 日 |
...
| 注意事项 | 添加或更新日期 | 描述 |
| ----------- | ---------- | ---------- |
| **购买 ID 和事件 ID（事件序列化）有效期为 37 个月** | 2023 年 5 月 25 日 | 即将发布的 Analytics Hit 处理引擎（针对 **2023 年 6 月下旬或 2023 年 7 月上旬**&#x200B;发布的版本）将开始实施购买 ID 和事件 ID（事件序列化）的 37 个月有效期。目前，购买 ID 和事件 ID 在 Adobe Analytics 中永不过期。一旦购买 ID 或事件 ID 被看到/使用，则任何未来点击（无论何时）都会将该购买或事件标记为重复项。在新的处理引擎发布后：<ul><li>购买 ID 和事件 ID 始终在 37 个月后过期。</li><li>如果自看到购买 ID 或事件 ID 以来已过去 37 个月，则不再将其视为重复购买或事件。</li><li> 如果您“重复使用”37 个月前的购买 ID 或事件 ID，则它们不再被视为重复项。</li></ul> |
...
| **伦敦数据中心的 Adobe Analytics 数据馈送和 Data Warehouse 出口使用新 IP** | 2023 年 4 月 27 日 | 这涉及伦敦数据中心的客户，他们将数据馈送请求和/或 Data Warehouse 报告传送到 FTP/SFTP 服务。应将以下 IP 地址范围添加到防火墙配置以允许访问： <ul><li>130.248.244.32/29</li><li>130.248.244.40/29</li></ul> |
