...
## 通过数据源上载我的数据时，为何不能同时获得我的量度？ {#metrics}
...
## 我可以向生成的模板添加更多事件还是只限于三个事件？ {#template}
...
## 独特访客在网站服务器日志数据源上载中是如何计算的？ {#unique}
...
## 使用交易 ID 导入的量度是否可以在 Clickstream 数据馈送和数据仓库中使用？ {#clickstream}

数据馈送包含已接收到的任何交易 ID 量度。但是，如果您上载过去某个日期的交易 ID 数据，则获取该数据的唯一方法是重新下载该日期的数据馈送。

## 当前保留在访客资料中的 eVar 是否分配到了使用数据源上载的量度中？ {#visitor}
...
## 使用数据源上载的 eVar 是否持续保留到后续在线行为？ {#evars}
