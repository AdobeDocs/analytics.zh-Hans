---
description: 此 Adobe® Data Connectors™ 电子邮件集成将 Analytics® 中的行为信息与 emarsys 电子邮件营销整合在一起，创建出一款功能强大的工具，通过更相关的消息传送来重新定义成功衡量指标和目标受众。
title: 适用于 Adobe Analytics 的 Emarsys Data Connector
...
# 适用于 Adobe Analytics 的 Emarsys Data Connector{#emarsys-data-connector-for-adobe-analytics}

>[!IMPORTANT]
>
>我们将在2021年中后期终止Adobe Data Connector技术。 [了解更多...](/help/import/data-connectors/data-connectors-eol.md)

此 Adobe® Data Connectors™ 电子邮件集成将 Analytics® 中的行为信息与 emarsys 电子邮件营销整合在一起，创建出一款功能强大的工具，通过更相关的消息传送来重新定义成功衡量指标和目标受众。
...
提高营销效率，是 [!DNL Analytics] 与 emarsys 集成的主要优势之一。此外，此集成会自动将电子邮件量度与 [!DNL Analytics] 数据同步（频率为每小时一次），以实现闭环报告。
...
这样做可确保在激活之前，遵循相应的最佳实践并满足相关先决条件，从而成功实现最佳集成。查看关于此 Data Connectors 集成的以下信息，这些信息与 emarsys 相关：

* **有效的 emarsys 帐户：**&#x200B;要使用此 Data Connectors 电子邮件集成，客户必须拥有有效的 emarsys 帐户。
* **emarsys 的当前客户：**&#x200B;此集成要求您同时是 Adobe 客户和 emarsys 客户。如果您当前不是 emarsys 客户，则不具备完成集成向导所需的信息。如果您当前是 emarsys 客户，则需要您的 emarsys 帐户 ID 或分配给贵组织的唯一标识符，才能完成集成向导。
* 完成 Data Connectors 向导后，请联系 emarsys 客户经理以在 emarsys eMarketing Suite 中激活该集成。
...
* **收件人 ID：**&#x200B;此集成要求我们捕获“访客 ID”并将其存储在一个 Analytics 变量 (eVar) 中。访客 ID（通常称为“收件人 ID”）是 emarsys 系统中电子邮件地址的编码或数字表示形式。此“收件人 ID”与提取到 emarsys 系统中的网站下游访客行为（放弃购买、购买等）相关联，且可用于再营销。在设置过程中，当收到向导提示时，您必须为此确定一个 eVar。
* **外部跟踪：**&#x200B;如果您当前没有遵循为您发送的每个电子邮件促销活动启用外部跟踪这一最佳实践，则必须这样做以确保成功集成。有关详细信息，请参阅下面的 emarsys 部分。
