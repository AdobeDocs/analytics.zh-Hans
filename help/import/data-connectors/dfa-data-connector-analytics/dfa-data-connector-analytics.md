...
**[!UICONTROL Analytics]** > **[!UICONTROL 管理]** > **[!UICONTROL Data connectors]**
...
如果发生超时，页面代码可指定一个 Adobe 报告与分析事件以用作超时事件。此事件可用于诊断集成问题，或调整 `s.maxDelay`。如果存在过多的超时，请增加 `s.maxDelay`。`s.maxDelay` 设置得过高时，访客可能会在 `s.maxDelay` 计时器到期前离开网站。
