...
通过数据馈送管理器，您可以为贵组织创建、编辑和取消数据馈送。 如果您有权访问数据馈送管理器，则可以管理对您可见的所有报表包的数据馈送。
...
1. （可选）要查看特定数据馈送的作业历史记录，请选中数据馈送旁边的复选框，然后选择&#x200B;[!UICONTROL **作业历史记录**]。
...
1. 选中要复制的数据馈送旁边的复选框，然后选择&#x200B;[!UICONTROL **复制**]。
   ...
   这会将您引导至[使用当前馈送的所有设置创建新馈送](create-feed.md)。 如果选择多个数据馈送，则复制选项不可见。
...
当您取消数据馈送时，其状态将设置为[!UICONTROL 已取消]。 数据馈送的状态必须为[!UICONTROL 活动]，然后才能取消。

要取消数据馈送，请执行以下操作：

1. 在 Adobe Analytics 中，选择&#x200B;[!UICONTROL **管理员**] > [!UICONTROL **数据馈送**]。

1. 选中要取消的数据馈送旁边的复选框，然后选择&#x200B;[!UICONTROL **取消**]。

## 排序和自定义数据馈送管理器中的列

每个数据馈送都会显示若干列，列中提供了有关馈送的信息。 您可以对每列中的信息进行排序，并自定义显示的列。
