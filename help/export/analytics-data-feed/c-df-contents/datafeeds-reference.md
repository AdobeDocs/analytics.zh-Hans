...
| `mc_audiences` | 列出访客所属的 Audience Manager 区段 ID。的 `post_mc_audiences` 列将分隔符更改为 `--**--`. | 文本 |
...
| `mvvar1` - `mvvar3` | 列出变量值。包含分隔的自定义值列表（取决于实施）。的 `post_mvvar1` - `post_mvvar3` 列将原始分隔符替换为 `--**--`. | 文本 |
