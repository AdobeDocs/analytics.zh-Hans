...
> [!IMPORTANT] 对于任何给定列（例如，定义为255个字符的列），由于在字符串中添加了字符转义值，数据馈送可以发送其他字符。 如果您的实施定期发送超过字符限制的值，请记住这些可能的额外字符。

## 列、说明和数据类型

> [!NOTE] 大多数列都包含一个带前缀的类似列 `post_`。 post 列包含应用服务器端逻辑、处理规则和 VISTA 规则后得出的值。大多数情况下，Adobe 建议使用 post_ 列。有关更 [多信息，请参阅数据源常见问题](../df-faq.md) 。
...
| `cookies` | 在“Cookie 支持”维度中使用的变量。<br>Y:启<br>用N:禁<br>用U:未知 | char(1) |
...
| `mobileappperformancecrashid` | 用于Apteligent数据连接器。 Apteligent中使用的崩溃ID。 | varchar(255) |
...
| `mobiledeeplinkid` | 从上下文数据变量a.<span>deeplink</span>.id收集。 在客户获取报告中用作移动客户获取链接的标识符。 | varchar(255) |
...
| `videoaudiostation` | 视频音频站 | varchar(255) |
