...
| `browser` | 浏览器的数字 ID。引用`browser.tsv`查找表。 | int，无符号 |
| `browser_height` | 浏览器窗口的高度（单位：像素）。 | smallint，无符号 |
| `browser_width` | 浏览器窗口的宽度（单位：像素）。 | smallint，无符号 |
| `c_color` | 调色板的位深度。用作计算[颜色深度](/help/components/dimensions/color-depth.md)维度的一部分。 AppMeasurement使用JavaScript函数`screen.colorDepth()`。 | char(20) |
...
| `channel` | 在[站点区域](/help/components/dimensions/site-section.md)维度中使用的变量。 | varchar(100) |
...
| `color` | 根据`c_color`列的值确定颜色深度ID。 引用`color_depth.tsv`查找表。 | smallint，无符号 |
| `connection_type` | 表示连接类型的数字 ID。在[连接类型](/help/components/dimensions/connection-type.md)维度中使用的变量。 引用`connection_type.tsv`查找表。 | tinyint，无符号 |
| `cookies` | 在[Cookie支持](/help/components/dimensions/cookie-support.md)维度中使用的变量。<br>Y：启用<br>N：禁用<br>U：未知 | char(1) |
| `country` | 表示点击来源的国家/地区的数字 ID。在[国家/地区](/help/components/dimensions/countries.md)维度中使用。 使用`country.tsv`查找。 | smallint，无符号 |
...
| `domain` | 在[域](/help/components/dimensions/domain.md)维度中使用的变量。 基于访客的Internet接入点。 | varchar(100) |
...
| `ef_id` | 在Adobe Advertising Cloud集成中使用的`ef_id`。 | varchar(255) |
...
| `exclude_hit` | 指示从报告中排除点击的标志。 排除的点击不会递增`visit_num`列。<br>1：未使用。包含在已弃用的功能中。<br>2：未使用。包含在已弃用的功能中。<br>3：已不再使用。用户代理排除<br>4:基于IP地址的排除<br>5:缺少重要点击信息，如`page_url`、`pagename`、`page_event`或`event_list`<br>6:JavaScript未正确处理点击<br>7:特定于帐户的排除，如VISTA规则<br>8:未使用。 替代特定于帐户的排除。<br>9：未使用。包含在已弃用的功能中。<br>10：无效的货币代码<br>11：仅时间戳报表包上缺少时间戳的点击，或非时间戳报表包上包含时间戳的点击<br>12：未使用。包含在已弃用的功能中。<br>13：未使用。包含在已弃用的功能中。<br>14：与 Analytics 点击不匹配的 Target 点击<br>15：当前未使用。<br>16：与 Analytics 点击不匹配的 Advertising Cloud 点击 | tinyint，无符号 |
| `first_hit_page_url` | 访客访问的第一个 URL。 | varchar(255) |
| `first_hit_pagename` | 在[登入页原始](/help/components/dimensions/entry-dimensions.md)维度中使用的变量。 访客访问的原始登录页面名称。 | varchar(100) |
| `first_hit_ref_domain` | 在[原始引用域](/help/components/dimensions/original-referring-domain.md)维度中使用的变量。 基于`first_hit_referrer`。 访客访问的第一个反向链接的域名。 | varchar(100) |
...
| `geo_city` | 点击来源的城市的名称（基于 IP）。用于[城市](/help/components/dimensions/cities.md)维度。 | char(32) |
| `geo_country` | 点击来源的国家/地区的缩写（基于 IP）。 | char(4) |
| `geo_dma` | 点击来源的人口统计区的数字 ID（基于 IP）。在[US DMA](/help/components/dimensions/us-dma.md)维度中使用。 | int，无符号 |
| `geo_region` | 点击来源的州/市/自治区或区域的名称（基于 IP）。用于[Regions](/help/components/dimensions/regions.md)维。 | char(32) |
...
| `hitid_high` | 与`hitid_low`结合使用，以唯一标识点击。 | bigint，无符号 |
| `hitid_low` | 与`hitid_high`结合使用，以唯一标识点击。 | bigint，无符号 |
...
| `javascript` | 基于`j_jscript`的JavaScript版本的查找ID。 使用查询表。 | tinyint，无符号 |
| `language` | 语言的数字 ID。使用`languages.tsv`查找表。 | smallint，无符号 |
| `last_hit_time_gmt` | 上次点击的时间戳（基于 Unix 时间）。用于计算自上次访问](/help/components/dimensions/days-since-last-visit.md)以来的[天数。 | int |
| `last_purchase_num` | 在[客户忠诚度](/help/components/dimensions/customer-loyalty.md)维度中使用的变量。 访客之前购买的次数。 <br>0：之前没有购买（不是客户）<br>1：1 次先前购买（新客户）<br>2：2 次先前购买（退货客户）<br>3：3 次或更多先前购买（忠诚客户） | int，无符号 |
| `last_purchase_time_gmt` | 用于上次购买](/help/components/dimensions/days-since-last-purchase.md)维度后的[天数。 上次购买的时间戳（基于 Unix 时间）。对于首次购买及之前未购买过的访客，该值为 `0`. | int |
...
| `mobileaction` | 移动设备操作。在Mobile Services中调用`trackAction`时自动收集。 应用程序支持自动的操作路径。 | varchar(100) |
...
| `os` | 表示访客操作系统的数字 ID。基于`user_agent`列。 使用`os`查找。 | int，无符号 |
| `p_plugins` | 已不再使用。可用于浏览器的插件列表。使用 JavaScript 函数 `navigator.plugins()`. | 文本 |
...
| `page_type` | 用于填充[找不到](/help/components/dimensions/pages-not-found.md)维度的页面。仅用于404页。 此变量应为空或包含值`ErrorPage`。 | char(20) |
| `page_url` | 点击的 URL。已从链接跟踪图像请求中去除。 | varchar(255) |
| `pagename` | 用于填充[Page](/help/components/dimensions/page.md)维度。 如果[`pagename`](/help/implement/vars/page-vars/pagename.md)变量为空，则Analytics会改用`page_url`。 | varchar(100) |
| `paid_search` | 设置点击是否与付费搜索检测匹配的标记。 | tinyint，无符号 |
| `partner_plugins` | 未使用。包含在已弃用的功能中。 | varchar(255) |
| `persistent_cookie` | 在[持久Cookie支持](/help/components/dimensions/persistent-cookie-support.md)维度中使用。 指示访客是否要支持每次点击后未被丢弃的 Cookie。 | char(1) |
...
| `prev_page` | 未使用。Adobe上一页的专有标识符。 | int，无符号 |
| `product_list` | 通过[`products`](/help/implement/vars/page-vars/products.md)变量传递的产品列表。 各产品用逗号分隔，而各个产品属性则用分号分隔。 | 文本 |
| `product_merchandising` | 未使用。请改用 `product_list`。 | 文本 |
| `prop1` -  `prop75` | 自定义流量变量 1 - 75。用于[Prop](/help/components/dimensions/prop.md)维度。 | varchar(100) |
| `purchaseid` | 购买的唯一标识符，使用[`purchaseID`](/help/implement/vars/page-vars/purchaseid.md)变量设置。 由`duplicate_purchase`列使用。 | char(20) |
| `quarterly_visitor` | 确定点击是否为新的每季访客的标记。 | tinyint，无符号 |
| `ref_domain` | 基于反向链接列。点击的反向链接域名。在[引荐域](/help/components/dimensions/referring-domain.md)维度中使用。 | varchar(100) |
| `ref_type` | 表示点击的反向链接类型的数字 ID。用于[推荐人类型](/help/components/dimensions/referrer-type.md)维度。 <br>1：网站内<br>2：其他网站<br>3：搜索引擎<br>4：硬盘<br>5：未发送<br>6：已输入/添加书签（无反向链接）<br>7：电子邮件<br>8：无 JavaScript <br>9：社交网络 | tinyint，无符号 |
| `referrer` | 上一页的页面 URL。用于[推荐人](/help/components/dimensions/referrer.md)维度。 请注意，在 `referrer` 使用数据类型 varchar(255) 时，`post_referrer` 使用数据类型 varchar(244)。 | varchar(255) |
| `resolution` | 表示显示器分辨率的数字 ID。用于[显示器分辨率](/help/components/dimensions/monitor-resolution.md)维度。 使用`resolution.tsv`查找表。 | smallint，无符号 |
| `s_kwcid` | Adobe Advertising Cloud 集成中使用的关键词 ID。 | varchar(255) |
| `s_resolution` | 原始屏幕分辨率值使用JavaScript函数`screen.width x screen.height`收集。 | char(20) |
| `search_engine` | 表示将访客引荐至您网站的搜索引擎的数字 ID。使用`search_engines.tsv`查找。 | smallint，无符号 |
| `search_page_num` | 由[所有搜索页面排名](/help/components/dimensions/all-search-page-rank.md)维使用。 指示用户在点进您网站之前您的网站位于搜索结果的第几页。 | smallint，无符号 |
| `secondary_hit` | 用于跟踪次级点击的标记。通常来自复制点击的多包标记和VISTA规则。 | tinyint，无符号 |
...
| `transactionid` | 稍后可以通过数据源上载各种数据点的唯一标识符。使用[`transactionID`](/help/implement/vars/page-vars/transactionid.md)变量收集。 | 文本 |
...
| `user_server` | 用于[Server](/help/components/dimensions/server.md)维。 | varchar(100) |
| `userid` | 已不再使用。表示报表包 ID 的数字 ID。请改用 `username`。 | int，无符号 |
| `username` | 点击的报表包 ID。 | char(40) |
| `va_closer_detail` | [上次触摸详细信息](/help/components/dimensions/last-touch-detail.md)维中使用的变量。 | varchar(255) |
| `va_closer_id` | 标识[上次触摸渠道](/help/components/dimensions/last-touch-channel.md)维度的数字ID。 可在营销渠道管理器中找到此 ID 所对应的查询表。 | tinyint，无符号 |
| `va_finder_detail` | 用于[首次触摸详细信息](/help/components/dimensions/first-touch-detail.md)维度的变量。 | varchar(255) |
| `va_finder_id` | 标识[首次触摸渠道](/help/components/dimensions/first-touch-channel.md)维度的数字ID。 可在营销渠道管理器中找到此 ID 所对应的查询表。 | tinyint，无符号 |
| `va_instance_event` | 用于标识营销渠道[实例](/help/components/metrics/instances.md)的标记。 | tinyint，无符号 |
| `va_new_engagement` | 用于标识营销渠道[新参与](/help/components/metrics/new-engagements.md)的标记。 | tinyint，无符号 |
...
| `visid_high` | 与`visid_low`结合使用，以唯一标识访客。 | bigint，无符号 |
| `visid_low` | 与`visid_high`结合使用，以唯一标识访客。 | bigint，无符号 |
| `visid_new` | 用于表示点击是否包含新生成的访客 ID 的标记。 | char(1) |
| `visid_timestamp` | 如果访客 ID 是新生成的，则会提供用于表示访客 ID 生成时间的时间戳（基于 Unix 时间）。 | int |
| `visid_type` | 不能用于外部用途；Adobe 内部用于处理优化。数字 ID，表示用于标识访客的方法。<br>0:自定义visitorID或未知/不适用<br>1:IP和用户代理回 <br>退2:HTTP移动订阅者 <br>标题3:旧版Cookie值(`s_vi`) <br>4:回退Cookie值(`s_fid`) <br>5:Identity Service | tinyint，无符号 |
| `visit_keywords` | 在[Search关键字](/help/components/dimensions/search-keyword.md)维度中使用的变量。 此列使用varchar(244)的非标准字符限制来容纳由Adobe使用的后端逻辑。 | varchar(244) |
| `visit_num` | 在[访问编号](/help/components/dimensions/visit-number.md)维度中使用的变量。 起始值为 1，随后每个访客每启动一次新访问，该值便会递增。 | int，无符号 |
| `visit_page_num` | 在[命中深度](/help/components/dimensions/hit-depth.md)维度中使用的变量。 用户每生成一次点击，该变量的值便会增加 1。每次访问后重置。 | int，无符号 |
| `visit_ref_domain` | 基于`visit_referrer`列。 访问中第一个反向链接的域名。 | varchar(100) |
| `visit_ref_type` | 数字 ID，表示访问中第一个反向链接的类型。使用`referrer_type.tsv`查找表。 | tinyint，无符号 |
