...
| **`browser_height`** | 此 [浏览器高度](/help/components/dimensions/browser-height.md) 维度。 | 无符号 smallint |
| **`browser_width`** | 此 [浏览器宽度](/help/components/dimensions/browser-width.md) | 无符号 smallint |
...
| **`channel`** | 此 [网站区域](/help/components/dimensions/site-section.md) 维度。 | varchar(100) |
...
| **`connection_type`** | 表示连接类型的数值ID。 此 [连接类型](/help/components/dimensions/connection-type.md) 维度。 引用 `connection_type.tsv` 查找表。 | 无符号 tinyint |
| **`cookies`** | 此 [Cookie支持](/help/components/dimensions/cookie-support.md) 维度。<br>Y：启用<br>N：禁用<br>U：未知 | char(1) |
...
| **`curr_factor`** | 确定货币的小数位，用于货币兑换。例如，USD使用两位小数，因此该列值将为 `2`. | tinyint |
| **`curr_rate`** | 交易时的汇率。Adobe 与 XE 合作，以确定当天的汇率。 | decimal(24,12) |
| **`currency`** | 交易期间使用的货币代码。 使用进行设置 [`currencyCode`](/help/implement/vars/config-vars/currencycode.md). | char(8) |
| **`cust_hit_time_gmt`** | 仅限启用了时间戳的报表包。随点击发送的时间戳（基于 UNIX® 时间）。 | int |
| **`cust_visid`** | 自定义访客ID（如果使用设置） [`visitorID`](/help/implement/vars/config-vars/visitorid.md). | varchar(255) |
| **`daily_visitor`** | 确定点击是否为新的每日访客的标记。 | 无符号 tinyint |
| **`dataprivacyconsentoptin`** | 此 [同意管理选择加入](/help/components/dimensions/cm-opt-in.md) 维度。 每次点击可以出现多个值，各个值之间用竖线 (`\|`) 隔开。有效值包括 `DMP` 和 `SELL`。 | varchar(100) |
| **`dataprivacyconsentoptout`** | 此 [同意管理选择退出](/help/components/dimensions/cm-opt-out.md) 维度。 每次点击可以出现多个值，各个值之间用竖线 (`\|`) 隔开。有效值包括 `SSF`, `DMP` 和 `SELL`。 | varchar(100) |
...
| **`evar1 - evar250`** | 自定义变量 1 至 250。在 [eVar](/help/components/dimensions/evar.md) 维度中用到。每个公司使用 eVar 的方式有所不同。要了解有关贵组织如何填充各个eVar的更多信息，最佳位置应该是 [解决方案设计文档](/help/implement/prepare/solution-design.md) 特定于您的组织。 | varchar(255) |
| **`event_list`** | 以逗号分隔的数字ID列表，表示点击时触发的事件。 包含默认事件和 [自定义事件1-1000](/help/components/metrics/custom-events.md). 使用 `event.tsv` 查找。 | 文本 |
| **`exclude_hit`** | 确定点击是否从报表中排除的标记。 对于被排除的点击，`visit_num` 列不递增。<br>1：未使用。属于某个已弃用的功能。<br>2：未使用。属于某个已弃用的功能。<br>3：已不再使用。用户代理排除<br>4：根据 IP 地址排除<br>5：缺少重要的点击信息，如 `page_url`、`pagename`、`page_event` 或 `event_list`<br>6：JavaScript 未正确处理点击<br>7：帐户特有的排除，如 VISTA 规则中的排除<br>8: 未使用。替代特定于帐户的排除。<br>9：未使用。属于某个已弃用的功能。<br>10：无效的货币代码<br>11：仅时间戳报表包上缺少时间戳的点击，或非时间戳报表包上包含时间戳的点击<br>12：未使用。属于某个已弃用的功能。<br>13：未使用。属于某个已弃用的功能。<br>14：与 Analytics 点击不匹配的 Target 点击<br>15：当前未使用。<br>16：与 Analytics 点击不匹配的 Advertising Cloud 点击 | 无符号 tinyint |
| **`first_hit_page_url`** | 访客访问的第一个 URL。 | varchar(255) |
| **`first_hit_pagename`** | 此 [原始登入页面](/help/components/dimensions/entry-dimensions.md) 维度。 访客访问的原始登录页面名称。 | varchar(100) |
| **`first_hit_ref_domain`** | 此 [原始反向链接域](/help/components/dimensions/original-referring-domain.md) 维度。 基于 `first_hit_referrer`。访客首次访问的反向链接域。 | varchar(100) |
...
| **`java_enabled`** | 此 [[!UICONTROL 已启用Java]](/help/components/dimensions/java-enabled.md). <br>Y：启用<br>N：禁用<br>U：未知 | char(1) |
| **`javascript`** | JavaScript版本的查找ID，基于 `j_jscript`. 引用 `javascript_version` 查找表。 | 无符号 tinyint |
...
| **`last_purchase_num`** | 此 [客户忠诚度](/help/components/dimensions/customer-loyalty.md) 维度。 访客以前购买的次数。<br>0：之前没有购买（不是客户）<br>1：1 次先前购买（新客户）<br>2：2 次先前购买（退货客户）<br>3：3 次或更多先前购买（忠诚客户） | 无符号 int |
...
| **`mobileaction`** | 移动设备操作。自动收集时间 `trackAction` 在移动设备实施中调用。 应用程序支持自动的操作路径。 | varchar(100) |
...
| **`mobileappstoreobjectid`** | 用于 [!DNL Appfigures] 数据连接器。 应用程序商店对象 ID。 | varchar(255) |
...
| **`mvvar1`** - `mvvar3` | [列表变量](/help/implement/vars/page-vars/list.md) 值。 包含分隔的自定义值列表（取决于实施）。`post_mvvar1` - `post_mvvar3` 列将原始分隔符替换为 `--**--`。 | 文本 |
...
| **`page_event_var2`** | 仅用于链接跟踪图像请求。链接的自定义名称（如果已指定）。 设置 [自定义链接](/help/components/dimensions/custom-link.md)， [下载链接](/help/components/dimensions/download-link.md)，或 [退出链接](/help/components/dimensions/exit-link.md) 取决于中的值 `page_event`. | varchar(100) |
...
| **`pagename`** | 此 [页面](/help/components/dimensions/page.md) 维度。 如果 [`pagename`](/help/implement/vars/page-vars/pagename.md) 变量为空，则 Analytics 改用 `page_url`。 | varchar(100) |
...
| **`product_list`** | [`products`](/help/implement/vars/page-vars/products.md)页面变量。帮助填充多个维度和指标，包括 [类别](/help/components/dimensions/category.md)、 [产品](/help/components/dimensions/product.md)、 [单位](/help/components/metrics/units.md)和 [收入](/help/components/metrics/revenue.md)。 | 文本 |
...
| **`ref_domain`** | 此 [反向链接域](/help/components/dimensions/referring-domain.md) 维度。 基于 `referrer` 列。 | varchar(100) |
...
| **`referrer`** | 此 [反向链接](/help/components/dimensions/referrer.md) 维度。 请注意，在 `referrer` 使用数据类型 varchar(255) 时，`post_referrer` 使用数据类型 varchar(244)。 | varchar(255) |
| **`resolution`** | 一个数字 ID，表示显示器的分辨率。 在[显示器分辨率](/help/components/dimensions/monitor-resolution.md)维度中用到。使用 `resolution.tsv` 查询表。 | 无符号 smallint |
...
| **`search_engine`** | 数字ID，表示将访客引荐至您的网站的搜索引擎。 使用位置 [搜索引擎](/help/components/dimensions/search-engine.md) 维度。 引用 `search_engines.tsv` 查找表。 | 无符号 smallint |
...
| **`va_closer_detail`** | 此 [最近联系详细信息](/help/components/dimensions/last-touch-detail.md) 维度。 | varchar(255) |
| **`va_closer_id`** | 数字ID，标识 [最近联系渠道](/help/components/dimensions/last-touch-channel.md) 维度。 此ID的查找可在营销渠道管理器中找到。 | 无符号 tinyint |
| **`va_finder_detail`** | 此 [首次联系详细信息](/help/components/dimensions/first-touch-detail.md) 维度。 | varchar(255) |
| **`va_finder_id`** | 数字ID，标识 [首个联系渠道](/help/components/dimensions/first-touch-channel.md) 维度。 此ID的查找可在营销渠道管理器中找到。 | 无符号 tinyint |
| **`va_instance_event`** | 标识营销渠道 [实例的](/help/components/metrics/instances.md)标志。 | 无符号 tinyint |
| **`va_new_engagement`** | 标识市场营销渠道 [新参与的](/help/components/metrics/new-engagements.md)标志。 | 无符号 tinyint |
...
| **`visit_keywords`** | 此 [搜索关键词](/help/components/dimensions/search-keyword.md) 维度。 此列使用 varchar(244) 的非标准字符限制容纳 Adobe 使用的后端逻辑。 | varchar(244) |
| **`visit_num`** | 此 [访问量](/help/components/dimensions/visit-number.md) 维度。 起始值为 1，每当每个访客开始新的访问时，此项就会递增。 | 无符号 int |
| **`visit_page_num`** | 此 [点击深度](/help/components/dimensions/hit-depth.md) 维度。 访客产生的每命中增加 1。 每次访问后重置。 | 无符号 int |
| **`visit_ref_domain`** | 基于 `visit_referrer` 列。访问中第一个反向链接的域名。 | varchar(100) |
| **`visit_ref_type`** | 表示访问的第一个反向链接的反向链接类型的数值ID。 引用 `referrer_type.tsv` 查找表。 | 无符号 tinyint |
...
| **`yearly_visitor`** | 确定点击是否为新的年度访客的标志。 | 无符号 tinyint |
