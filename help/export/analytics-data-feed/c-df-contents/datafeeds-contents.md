...
一些馈送配置为接收 `.fin` 文件，而不是 `.txt` 清单。`.fin`表示上传完成，但其包含的元数据采用旧格式。
...
* **`column_headers.tsv`**：包含`hit_data.tsv`的列标题的单个行。
* **`browser.tsv`**：将浏览器ID（`browser`信息源列）映射到浏览器的友好名称。
* **`browser_type.tsv`**：将浏览器ID（`browser`信息源列）映射到浏览器类型。
* **`color_depth.tsv`**：将颜色深度ID（`color`信息源列）映射到颜色深度。
* **`connection_type.tsv`**：将连接类型ID （`connection_type`馈送列）映射到连接类型。
* **`country.tsv`**：将国家/地区ID（`country`信息源列）映射到国家/地区名称。
* **`javascript_version.tsv`**：将JavaScript版本ID（`javascript`信息源列）映射到JavaScript版本。
* **`languages.tsv`**：将语言ID （`language`信息源列）映射到语言。
* **`operating_systems.tsv`**：将操作系统ID （`os`馈送列）映射到操作系统名称。
* **`plugins.tsv`**：将插件ID（`plugin`信息源列）映射到每个相应的插件名称。
* **`resolution.tsv`**：将分辨率ID（`resolution`信息源列）映射到监视器分辨率。
* **`referrer_type.tsv`**：将反向链接类型ID（`ref_type`馈送列）映射到反向链接类型。
* **`search_engines.tsv`**：将搜索引擎ID（`search_engine`信息源列）映射到搜索引擎名称。
* **`event.tsv`**：将每个事件ID（`event_list`信息源列）映射到其各自的事件名称。
...
* `[format_suffix]`引用文件格式类型。 通常数据馈送文件格式为`.tsv`。
...
提取后，每个数据文件都包含单个`[index]-[rsid]_[YYYYmmdd]-[HHMMSS].[format_suffix]`文件（其中包含约2GB未压缩数据）以及任何所需列的查询文件。
