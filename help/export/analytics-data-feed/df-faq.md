...
在处理每小时或每日数据之前，数据馈送会等待直到该时间范围（天或小时）内进入数据收集的所有点击已经写出到 Data Warehouse。 之后，数据馈送会收集时间戳位于该时间范围之内的数据，对其进行压缩并通过 FTP 发送。 对于每小时馈送，文件通常会在一小时后的15-30分钟内写出到Data Warehouse，但没有设置时间段。 如果没有时间戳的数据在该时间范围内，则该流程将在下一个时间范围内重试。 当前数据馈送流程使用 `date_time` 字段来确定哪些点击属于该小时。 此字段基于报告包的时区。
...
如果您在非后处理列和后处理列中看到同一个值的不同大小写（例如，前处理列中的“snow”与后处理列中的“Snow”），则说明您的实施在整个网站中同时使用了大写值和小写值。 后处理列中的不同大小写是在之前传递的，并被存储在虚拟 Cookie 中，或者是在大致相同的时间针对该报告包处理的。
...
对于特定时区，由于夏令时 (DST) 定义，每年时间要改变两次。 数据馈送遵循报告包所配置的时区。 如果报表包的时区不使用 DST 的时区，文件传递与其他日期一样正常继续。 如果报表包的时区使用DST的时区，则在更改时间的那个小时（通常是凌晨2:00）变更文件投放。

进行STD -> DST时间转换（调快一个小时）时，您会收到23个文件。 忽略 DST 过渡中跳过的那个小时。 例如，如果转换发生在凌晨2点，则您会收到1:00那个小时的文件和3:00那个小时的文件。 没有2:00的文件，因为2:00 STD变成了3:00 DST。

进行DST -> STD转换（回退）时，您会收到24个文件。 但是，进行过渡的那个小时实际包含两个小时的数据。 例如，如果过渡在凌晨2:00进行，则1:00那个小时的文件将延迟一个小时，但包含两个小时的数据。 它包含从1:00 DST到2:00 STD（实际是3:00 DST）的数据。 下一个文件从2:00 STD开始。
...
Amazon S3 的常见用例是 Amazon Web Services (AWS) 帐户所有者创建一个存储桶，接着创建一个有权限在该存储桶中创建对象的用户，然后为该用户提供凭据。 在这种情况下，一个用户的对象属于相同帐户，而帐户所有者隐式拥有对象的完整控制权限（读取、删除等）。 此过程类似于 FTP 传递的工作方式。
...
但是，对象不从父存储桶继承权限。 因此，如果用户 B 上传对象到用户 A 的存储桶，用户 B 仍然“拥有”该对象，默认情况下不向用户 A 授予对该对象的任何权限，虽然用户 A 拥有该存储桶。 用户 B 必须明确赋予用户 A 权限，因为用户 B 仍然是该对象的所有者。 要授予此权限，userB 必须上传具有 BucketOwnerFullControl ACL 的对象，该 ACL 指定向存储桶所有者（userA）授予对该对象的完整权限（读取、写入、删除等），即使该对象由 userB“拥有”。
