...
description: 本指南概述了如何使用数据馈送和 Data Warehouse 从 Adobe Analytics 中导出数据。
...
* **数据馈送：**&#x200B;每小时或每天收到的原始数据导出结果。 每一行都是单独的点击，每一列都是一个变量。 数据馈送通常发送到 [云目标](/help/export/analytics-data-feed/create-feed.md)。 虽然旧版 FTP 和 SFTP 帐户可用，但建议不要使用这些帐户。
* **Data Warehouse：**&#x200B;使用请求向导，以电子表格的形式重新获取输出的数据。 Data Warehouse 使用不同的处理架构，以支持任意数量的行和任意数量的唯一值。 Data Warehouse 请求通常发送到[云目标](/help/export/data-warehouse/create-request/dw-request-report-destinations.md)。 虽然旧版 FTP 和 SFTP 帐户可用，但建议不要使用这些帐户。
...
* [为 Data Warehouse 请求配置报表目标](/help/export/data-warehouse/create-request/dw-request-report-destinations.md)
